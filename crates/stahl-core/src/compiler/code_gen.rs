use super::{
    ...
    program::number_literal_to_stahl,
};
use crate::{
...
/// Converts a syntax object's token into a `StahlVal` or returns an error if it is not a valid
/// `StahlVal`.
...
fn try_eval_atom(t: &SyntaxObject) -> Option<StahlVal> {
    match &t.ty {
        TokenType::BooleanLiteral(b) => Some((*b).into()),
        TokenType::Number(n) => number_literal_to_stahl(n).ok(),
        TokenType::StringLiteral(s) => Some(StahlVal::StringV(s.to_string().into())),
        ...
        TokenType::Keyword(k) => Some(StahlVal::SymbolV(k.clone().into())),
        _what => {
            return None;
        }
    }
...
fn try_eval_atom_with_context(t: &SyntaxObject) -> Result<StahlVal> {
    match &t.ty {
        TokenType::BooleanLiteral(b) => Ok((*b).into()),
        TokenType::Number(n) => number_literal_to_stahl(n).map_err(|e| e.with_span(t.span)),
...
impl<'a> CodeGenerator<'a> {
    ...
    fn specialize_call(&mut self, l: &List, op: OpCode) -> Option<()> {
        ...
        // if value == StahlVal::IntV(1) && op == OpCode::SUBREGISTER {
...
impl<'a> VisitorMut for CodeGenerator<'a> {
    ...
    fn visit_quote(&mut self, quote: &crate::parser::ast::Quote) -> Self::Output {
        let converted =
            StahlVal::try_from(crate::parser::ast::ExprKind::Quote(Box::new(quote.clone())))?;
        ...
        self.push(
            LabeledInstruction::builder(OpCode::PUSHCONST)
                ...
                // embedding the stahl val directly here.
    ...
    fn visit_vector(&mut self, v: &crate::parser::ast::Vector) -> Self::Output {
        for arg in &v.args {
            if v.bytes {
                ...
                let converted = TryFromExprKindForStahlVal::try_from_expr_kind_quoted(arg.clone())?;
