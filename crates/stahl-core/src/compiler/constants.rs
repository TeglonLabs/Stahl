...
use crate::rvals::{into_serializable_value, Result, SerializableStahlVal, StahlVal};
...
use stahl_parser::parser::{lower_entire_ast, SourceId};
...
pub struct ConstantMap {
    map: SharedMut<HashMap<StahlVal, usize>>,
    values: SharedMut<Vec<StahlVal>>,
    // TODO: Flush to these values after a compilation. - maybe have two of them to
    reified_values: Arc<ArcSwap<Vec<StahlVal>>>,
}
...
impl ConstantMap {
    ...
    pub fn from_vec(vec: Vec<StahlVal>) -> ConstantMap {
        ConstantMap {
            map: Shared::new(MutContainer::new(
                vec.clone()
                    .into_iter()
    ...
    pub fn from_bytes(encoded: &[u8]) -> Result<ConstantMap> {
        ...
        str_vector
            ...
            .map(|x| {
                ...
                TryFromExprKindForStahlVal::try_from_expr_kind(parsed[0].clone())
...
impl ConstantMap {
    pub fn add(&mut self, val: StahlVal) -> usize {
        let idx = self.len();
    ...
    // Fallible
    #[inline(always)]
    pub fn get(&self, idx: usize) -> StahlVal {
        self.values.read()[idx].clone()
    }

    pub fn get_value(&self, idx: usize) -> StahlVal {
        self.reified_values.load()[idx].clone()
    }

    pub fn try_get(&self, idx: usize) -> Option<StahlVal> {
        self.values.read().get(idx).cloned()
    }

    // Replace with existing constants if they already exist
    fn walk_constants(&mut self, val: &StahlVal) -> Option<StahlVal> {
        match val {
            StahlVal::ListV(l) => Some(StahlVal::ListV(
                l.iter()
                    .map(|value| {
    ...
    // questionable
    pub fn add_or_get(&mut self, mut val: StahlVal) -> usize {
        if let StahlVal::ListV(_) = &val {
...
pub mod constant_table_tests {
    ...
    fn test_add(instance: &mut ConstantMap) {
        ...
        let val1 = StahlVal::BoolV(true);
        let val2 = StahlVal::BoolV(false);
    ...
    fn test_get(instance: &mut ConstantMap) {
        ...
        assert_eq!(instance.get(0), StahlVal::BoolV(true));
        assert_eq!(instance.get(1), StahlVal::BoolV(false));
    }
}
