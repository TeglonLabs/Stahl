...
use stahl_parser::{ast::PROTO_HASH_GET, expr_list, parser::SourceId, span::Span};
...
use crate::parser::expander::StahlMacro;
use crate::stop;
...
declare_builtins!(
    "stahl/option" => "../scheme/modules/option.scm",
    "stahl/result" => "../scheme/modules/result.scm",
    "stahl/iterators" => "../scheme/modules/iterators.scm",
    "stahl/mutable-vectors" => "../scheme/modules/mvector.scm",
    "stahl/async" => "../scheme/modules/async.scm",
    "stahl/sync" => "../scheme/modules/sync.scm",
    "#%private/stahl/contract" => "../scheme/modules/contracts.scm",
    "#%private/stahl/print" => "../scheme/print.scm",
    "#%private/stahl/control" => "../scheme/modules/parameters.scm",
    "#%private/stahl/reader" => "../scheme/modules/reader.scm",
    "#%private/stahl/stdlib" => "../scheme/stdlib.scm",
    "#%private/stahl/match" => "../scheme/modules/match.scm"
);

create_prelude!(
    "#%private/stahl/control",
    "#%private/stahl/contract",
    "#%private/stahl/print",
    "#%private/stahl/reader",
    "#%private/stahl/match",
    for_syntax "#%private/stahl/control",
    for_syntax "#%private/stahl/contract"
);

#[cfg(not(target_arch = "wasm32"))]
pub static STAHL_HOME: Lazy<Option<String>> = Lazy::new(|| {
    std::env::var("STAHL_HOME").ok().or_else(|| {
...
pub static STAHL_HOME: Lazy<Option<String>> = Lazy::new(|| None);

pub fn stahl_home() -> Option<String> {
    STAHL_HOME.clone()
}
...
impl ModuleManager {
    ...
    pub(crate) fn add_module(
        &mut self,
        path: PathBuf,
        global_macro_map: &mut FxHashMap<InternedString, StahlMacro>,
    ...
    fn find_in_scope_macros<'a>(
        ...
        &'a CompiledModule,
        FxHashMap<InternedString, StahlMacro>,
        NameMangler,
    ) {
...
pub struct CompiledModule {
    ...
    pub(crate) macro_map: FxHashMap<InternedString, StahlMacro>,
    ast: Vec<ExprKind>,
    emitted: bool,
...
pub fn path_to_module_name(name: PathBuf) -> String {
    ...
    if let Some(stahl_home) = STAHL_HOME.as_ref() {
        // Intern this?
        let name = name
            .to_str()
            .unwrap()
            .trim_start_matches(stahl_home.as_str());
...
impl CompiledModule {
    pub fn new(
        ...
        require_objects: Vec<RequireObject>,
        provides_for_syntax: Vec<InternedString>,
        macro_map: FxHashMap<InternedString, StahlMacro>,
        ast: Vec<ExprKind>,
    ) -> Self {
    ...
    fn to_top_level_module(
        ...
        global_macro_map: &FxHashMap<InternedString, StahlMacro>,
    ) -> Result<ExprKind> {
        ...
        // the path from the $STAHL_HOME would be relatively safe. That way we can just strip
        // the $STAHL_HOME root away from the path if it starts with it, and then have
    ...
    fn _to_module_ast_node(&self) -> ExprKind {
        ...
        // let stahl_base = ExprKind::List(List::new(vec![ExprKind::atom("stahl/base".to_string())]));

        // self.ast.pretty_print();

        // body.push(stahl_base);
...
struct ModuleBuilder<'a> {
    ...
    builtin_modules: ModuleContainer,
    global_macro_map: &'a FxHashMap<InternedString, StahlMacro>,
        ...
        if self.require_objects.is_empty() && !self.main {
                ...
                .map(|x| (x.path.get_path(), x.span))
            {
                ...
                let last_modified = std::fs::metadata(module.as_ref())
                    .map_err(|err| {
                        let mut err = crate::StahlErr::from(err);
        ...
        self.source_ast.retain_mut(|expr| {
            if let ExprKind::Macro(_) = expr {
                ...
                if let ExprKind::Macro(m) = taken_expr {
                    match StahlMacro::parse_from_ast_macro(m) {
                        Ok(generated_macro) => {
    ...
    fn parse_require_object_inner(
        ...
        match atom {
            ExprKind::Atom(Atom {
                ...
                // // But theres the case where we're searching for a module on the STAHL_HOME
                if !current.exists() {
                    if let Some(mut home) = home.clone() {
                        home.push(PathBuf::from(s.as_str()));
                        current = home;

                        log::info!("Searching STAHL_HOME for {:?}", current);
                        ...
                        // TODO: Check if this module exists in STAHL_HOME first. If it does, we'll take that as our candidate
                        ...
                        stop!(Generic => format!("Module not found: {:?} with STAHL_HOME: {:?}", current, home); *span)
                    }
                }
