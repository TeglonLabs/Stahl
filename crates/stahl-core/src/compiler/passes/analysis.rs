...
use crate::{
    ...
    stahl_vm::primitives::{builtin_to_reserved, MODULE_IDENTIFIERS},
...
impl<'a> VisitorMutUnitRef<'a> for AnalysisPass<'a> {
        ...
        if let Some(ident) = name {
            ...
            if let Some(depth) = self.info.scope.height_of(ident) {
                if depth == 0 {
                    ...
                    if crate::stahl_vm::primitives::PRELUDE_INTERNED_STRINGS
                        .with(|x| x.contains(ident))
                    {
                        semantic_information.mark_builtin()
                    }

                    // if crate::stahl_vm::primitives::PRELUDE_MODULE
            ...
            if let Some(analysis) = self.info.info.get_mut(&a.syn.syntax_object_id) {
                ...
                // TODO: We _really_ should be providing the built-ins in a better way thats not
                // passing around a thread local
                // if crate::stahl_vm::primitives::PRELUDE_MODULE.with(|x| x.contains(ident.resolve()))
                if crate::stahl_vm::primitives::PRELUDE_INTERNED_STRINGS.with(|x| x.contains(ident))
                {
                    semantic_info.mark_builtin();
    ...
    fn visit_vector(&mut self, _: &'a stahl_parser::ast::Vector) {}
}
...
struct FunctionCallCollector<'a> {
    ...
    constants: ImmutableHashMap<InternedString, StahlVal, FxBuildHasher>,
    should_mangle: bool,
}
...
impl<'a> SemanticAnalysis<'a> {
    ...
    pub fn inline_function_call<A: AsRef<str>>(&mut self, name: A) -> Result<(), StahlErr> {
    ...
    pub(crate) fn replace_non_shadowed_globals_with_builtins(
        &mut self,
        macros: &mut FxHashMap<InternedString, StahlMacro>,
        ...
        for stahl_macro in macros.values_mut() {
            if !stahl_macro.is_mangled() {
                for expr in stahl_macro.exprs_mut() {
                    macro_replacer.visit(expr);
                }

                stahl_macro.mark_mangled();
            }
        }

        for module in module_manager.modules_mut() {
            for stahl_macro in module.1.macro_map.values_mut() {
                if !stahl_macro.is_mangled() {
                    for expr in stahl_macro.exprs_mut() {
                        macro_replacer.visit(expr);
                    }
    ...
    pub(crate) fn remove_unused_globals_with_prefix(
        &mut self,
        prefix: &str,
        macros: &FxHashMap<InternedString, StahlMacro>,
        ...
        let stahl_constant_module: InternedString = "%-builtin-module-stahl/constants".into();
        ...
        for stahl_macro in macros.values() {
            // println!("Visiting: {}", stahl_macro.name().resolve());
            for expr in stahl_macro.exprs() {
        ...
        for module in module_manager.modules() {
            ...
            for stahl_macro in module.1.macro_map.values() {
                // println!("Visiting: {}", stahl_macro.name().resolve());
                // if !stahl_macro.is_mangled() {
                for expr in stahl_macro.exprs() {
                    collected.visit(expr);
                        ...
                        if let ExprKind::Define(define) = expression {
                                ...
                                if name.resolve().starts_with(prefix) {
                                        ...
                                        .and_then(|x| self.analysis.get(x))
                                    {
                                        if analysis.usage_count == 0 {
                                            ...
                                            {
                                                // (%module-get%
                                                //         %-builtin-module-stahl/constants
                                                //         (quote
                                                //           void))
                            ...
                            if let ExprKind::List(l) = expression {
                                if let Some(func) = l.first_ident() {
                                    if *func == module_get_interned {
                                        if l[1].atom_identifier() == Some(&stahl_constant_module) {
    ...
    pub fn check_if_values_are_redefined(&mut self) -> Result<&mut Self, StahlErr> {
  ...
  (λ ()
    ...
    fn test_black_box_mangling() {
        ...
        let mut exprs = Parser::parse(script).unwrap();
        {
            ...
            constants.insert("+".into(), StahlVal::Void);
            constants.insert("<=".into(), StahlVal::Void);
            constants.insert("-".into(), StahlVal::Void);
