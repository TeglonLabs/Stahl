...
use crate::rvals::{Result, StahlComplex};
use crate::{
...
use stahl_parser::tokens::{IntLiteral, NumberLiteral, RealLiteral};
...
pub fn number_literal_to_stahl(n: &NumberLiteral) -> Result<StahlVal> {
    // real_to_stahl does some cloning of bignums. It may be possible to optimize this away.
    let real_to_stahl = |re: &RealLiteral| match re {
        RealLiteral::Int(IntLiteral::Small(i)) => i.into_stahlval(),
        RealLiteral::Int(IntLiteral::Big(i)) => i.clone().into_stahlval(),
        RealLiteral::Rational(n, d) => match (n, d) {
            (IntLiteral::Small(n), IntLiteral::Small(d)) => {
                match (i32::try_from(*n), i32::try_from(*d)) {
                    ...
                    (Ok(n), Ok(d)) => Rational32::new(n, d).into_stahlval(),
                    _ => BigRational::new(BigInt::from(*n), BigInt::from(*d)).into_stahlval(),
                }
            }
            (IntLiteral::Small(n), IntLiteral::Big(d)) => {
                BigRational::new(BigInt::from(*n), *d.clone()).into_stahlval()
            }
            (IntLiteral::Big(n), IntLiteral::Small(d)) => {
                BigRational::new(*n.clone(), BigInt::from(*d)).into_stahlval()
            }
            (IntLiteral::Big(n), IntLiteral::Big(d)) => {
                BigRational::new(*n.clone(), *d.clone()).into_stahlval()
            }
        },
        RealLiteral::Float(f) => f.into_stahlval(),
    };
    match n {
        NumberLiteral::Real(re) => real_to_stahl(re),
...
fn eval_atom(t: &SyntaxObject) -> Result<StahlVal> {
    match &t.ty {
        ...
        TokenType::StringLiteral(s) => Ok(StahlVal::StringV(s.to_string().into())),
        ...
        match (instruction, next) {
            ...
            (
                ...
                let opcode = match &value {
                    StahlVal::IntV(0) => OpCode::LOADINT0POP,
                    StahlVal::IntV(1) => OpCode::LOADINT1POP,
                    StahlVal::IntV(2) => OpCode::LOADINT2POP,
                    _ => continue,
                };
            ...
            (
                ...
                let value = eval_atom(&syn)?;
                let opcode = match &value {
                    StahlVal::IntV(0) => OpCode::LOADINT0,
                    StahlVal::IntV(1) => OpCode::LOADINT1,
                    StahlVal::IntV(2) => OpCode::LOADINT2,
                    _ => continue,
                };
                instructions.get_mut(i).unwrap().op_code = opcode;
            }
...
pub const fn sequence_to_opcode(pattern: &[(OpCode, usize)]) -> &'static [stahl_gen::Pattern] {
    match pattern {
        &[(OpCode::MOVEREADLOCAL, _)] => &[stahl_gen::Pattern::Single(OpCode::MOVEREADLOCAL)],
        _ => todo!(),
    }
}
    ...
    #[cfg(feature = "dynamic")]
    {
        pub fn tile<const N: usize>(instructions: &mut [Instruction]) {
            ...
            for i in 0..instructions.len() - N {
                ...
                if let Some(op_code) = stahl_gen::opcode::sequence_to_opcode(&buffer) {
                    // Check if this pattern genuinely matches one of the code gen'd ones
                    stahl_gen::Pattern::from_opcodes_with_buffer(&buffer, &mut pattern_buffer);

                    if crate::stahl_vm::vm::pattern_exists(&pattern_buffer) {
...
impl RawProgramWithSymbols {
    ...
    //                 Ok(StahlVal::try_from(parsed[0].clone()).unwrap())
    //             })
    ...
    //         let value = StahlVal::try_from(parsed[0].clone()).unwrap();

    //         if let StahlVal::ListV(v) = value {
