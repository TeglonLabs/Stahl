...
// expands into
//      const LENGTH: StahlVal = StahlVal::FuncV(length)
macro_rules! declare_const_ref_functions {
    ($($name:tt => $func_name:tt),* $(,)? ) => {
        $ (
            pub(crate) const $name: StahlVal = StahlVal::FuncV($func_name);
        ) *
    }
}
...
// expands into
//      const LENGTH: StahlVal = StahlVal::MutFunc(length)
macro_rules! declare_const_mut_ref_functions {
    ($($name:tt => $func_name:tt),* $(,)? ) => {
        $ (
            pub(crate) const $name: StahlVal = StahlVal::MutFunc($func_name);
        ) *
    }
}
