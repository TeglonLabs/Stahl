...
pub struct Env {
    ...
    pub(crate) bindings_vec: Vec<StahlVal>,
    ...
    #[cfg(feature = "sync")]
    // pub(crate) bindings_vec: Arc<RwLock<Vec<StahlVal>>>,
    ...
    // just by offset.
    #[cfg(feature = "sync")]
    pub(crate) thread_local_bindings: Vec<StahlVal>,
}
...
#[cfg(not(feature = "sync"))]
impl Env {
    pub fn extract(&self, idx: usize) -> Option<StahlVal> {
    ...
    pub(crate) fn _print_diagnostics(&self) {
        for (idx, value) in self.bindings_vec.iter().enumerate() {
            if let StahlVal::Closure(b) = value {
    ...
    #[inline(always)]
    pub fn repl_lookup_idx(&self, idx: usize) -> StahlVal {
        self.bindings_vec[idx].clone()
    }

    /// Get the value located at that index
    pub fn _repl_get_idx(&self, idx: usize) -> &StahlVal {
        &self.bindings_vec[idx]
    }

    #[inline]
    pub fn repl_define_idx(&mut self, idx: usize, val: StahlVal) {
        if idx < self.bindings_vec.len() {
            ...
            if idx > self.bindings_vec.len() {
                ...
                for _ in 0..(idx - self.bindings_vec.len()) {
                    self.bindings_vec.push(StahlVal::Void);
                }
            }
    ...
    pub fn repl_set_idx(&mut self, idx: usize, val: StahlVal) -> Result<StahlVal> {
    ...
    #[inline]
    pub fn add_root_value(&mut self, idx: usize, val: StahlVal) {
        // self.bindings_map.insert(idx, val);
        self.repl_define_idx(idx, val);
    }

    pub fn roots(&self) -> &Vec<StahlVal> {
        &self.bindings_vec
    }
}
...
impl Env {
        ...
        if idx < self.thread_local_bindings.len() {
            ...
            if idx > self.thread_local_bindings.len() {
                ...
                for _ in 0..(idx - self.thread_local_bindings.len()) {
                    guard.push(StahlVal::Void);
                    self.thread_local_bindings.push(StahlVal::Void);
                }
            }
    ...
    #[cfg(feature = "sync")]
    pub fn roots(&self) -> RwLockReadGuard<'_, Vec<StahlVal>> {
