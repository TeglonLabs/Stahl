...
impl fmt::Display for Gc<StahlVal> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)
    }
}
...
impl<T> Gc<T> {
    ...
    pub fn try_new(val: T) -> Result<Gc<T>, StahlErr> {
    ...
    pub fn checked_allocate(allocations: usize) -> Result<(), StahlErr> {
    ...
    pub fn check_memory() -> Result<usize, StahlErr> {
        let mem: usize = OBJECT_COUNT.fetch_add(0, Ordering::SeqCst);
        if mem > MAXIMUM_OBJECTS {
            stop!(Generic => "ran out of memory!")
        }
...
pub mod unsafe_roots {
    ...
    fn test_rooting() {
        ...
        let root = Gc::new(StahlVal::ListV(im_lists::list![]));
...
pub mod unsafe_erased_pointers {
    ...
    */
    ...
    use crate::stahl_vm::engine::EngineId;
    use once_cell::sync::Lazy;
    use std::collections::HashMap;

    use crate::rvals::cycles::IterativeDropHandler;
    use crate::rvals::{AsRefStahlValFromRef, MaybeSendSyncStatic};
    use crate::{rerrs::ErrorKind, rvals::AsRefMutStahlValFromRef, StahlErr, StahlVal};
    ...
    impl<T> MutableReferenceNursery<T> {
        ...
        pub fn retain_readonly_reference(
            &mut self,
            original: &T,
            mut thunk: impl FnMut(ReadOnlyBorrowedObject<T>) -> crate::rvals::Result<StahlVal>,
        ) -> crate::rvals::Result<StahlVal> {
        ...
        pub fn retain_reference(
            &mut self,
            original: &mut T,
            mut thunk: impl FnMut(BorrowedObject<T>) -> crate::rvals::Result<StahlVal>,
        ) -> crate::rvals::Result<StahlVal> {
    ...
    // impl<T: ReferenceCustomType + 'static> IntoStahlVal for T {
    //     fn into_stahlval(self) -> crate::rvals::Result<StahlVal> {
    //         // Ok(self.new_stahl_val())
    //         Ok(StahlVal::Custom(Rc::new(RefCell::new(Box::new(self)))))
    //     }
    // }
    ...
    impl StahlVal {
        pub(crate) fn get_borrow_flag_if_borrowed_object<T: AsRefMutStahlValFromRef + 'static>(
            &self,
        ) -> crate::rvals::Result<Rc<Cell<bool>>> {
            if let StahlVal::Reference(v) = self {
        ...
        pub(crate) fn get_borrow_count_if_borrowed_object<T: AsRefMutStahlValFromRef + 'static>(
            &self,
    ...
    impl OpaqueReferenceNursery {
        ...
        pub(crate) fn drain_weak_references_to_stahlvals() -> Vec<StahlVal> {
            let res = NURSERY.with(|x| {
                x.weak_values
                    .write()
                    .drain(..)
                    .map(Gc::new)
                    .map(StahlVal::Reference)
                    .collect()
            });
    ...
    impl<T: ReferenceCustomType + 'static> AsRefMutStahlValFromRef for T {
        fn as_mut_ref_from_ref<'a>(val: &'a StahlVal) -> crate::rvals::Result<&'a mut T> {
            if let StahlVal::Reference(v) = val {
                let res = v.inner.as_any_ref();
    ...
    impl<T: ReferenceCustomType + 'static> AsRefStahlValFromRef for T {
        fn as_ref_from_ref<'a>(val: &'a StahlVal) -> crate::rvals::Result<&'a T> {
            if let StahlVal::Reference(v) = val {
                let res = v.inner.as_any_ref();
