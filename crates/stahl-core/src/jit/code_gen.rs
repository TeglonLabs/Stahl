...
thread_local! {
    pub static MEMORY: RefCell<Vec<Gc<StahlVal>>> = RefCell::new(Vec::new());
}
...
unsafe extern "C" fn length_list(value: i128) {
    ...
    let decoded = std::mem::transmute::<i128, StahlVal>(value);
...
unsafe extern "C" fn length(value: f64) -> f64 {
    ...
    // if let StahlVal::Pair(_) = lst {
    //     let count = StahlVal::iter(lst).count() as isize;
    // }
...
unsafe extern "C" fn empty_const() -> f64 {
    let empty_list = Gc::new(StahlVal::VectorV(Gc::new(Vector::new())));
...
unsafe extern "C" fn car(value: f64) -> f64 {
    // let lst: Box<StahlVal> = std::mem::transmute(value);

    todo!()

    // let lst = get_ref_from_double(value);

    // if let StahlVal::Pair(c) = lst {
    //     let ret_value = &c.car;
...
unsafe extern "C" fn cdr(value: f64) -> f64 {
    ...
    //         let new_pair = Gc::new(StahlVal::Pair(rest));
...
unsafe extern "C" fn empty(l: f64) -> f64 {
    ...
    encode_bool(if let StahlVal::VectorV(v) = lst {
        v.is_empty()
    } else {
        false
    })
...
unsafe extern "C" fn cons(car: f64, cdr: f64) -> f64 {
    ...
    // match &cdr {
    //     StahlVal::Pair(cdr) => {
    //         let new_value = Gc::new(StahlVal::Pair(Gc::new(ConsCell::new(
    //             car.clone(),
    //             Some(cdr.clone()),
    //         ))));
    ...
    //             Gc::new(StahlVal::Pair(Gc::new(ConsCell::new(car, None))))
    //         } else {
    //             Gc::new(StahlVal::Pair(Gc::new(ConsCell::new(
    //                 car,
...
//     if let (StahlVal::IntV(l), StahlVal::IntV(r)) = (left, right) {
//         to_encoded_double(StahlVal::IntV(l + r))
//     } else {
...
fn register_primitives(builder: &mut JITBuilder) {
    ...
    let outer = move |args: &[StahlVal]| -> StahlVal {
        println!("{}", outer_value);
...
impl JIT {
    ...
    pub(crate) fn allocate(value: &Gc<StahlVal>) {
...
impl<'a> FunctionTranslator<'a> {
    ...
    fn decode_float_or_stahlval_to_int(&mut self, value: Value) -> Value {
        // let bitmask
        unimplemented!()
    }
