...
use crate::StahlVal;
use crate::{gc::Gc, rvals::StahlString};
...
impl ToNanTaggedValue for &Gc<StahlVal> {
    fn encode(self) -> f64 {
        to_encoded_double(self)
    }
}

impl ToNanTaggedValue for &StahlVal {
    fn encode(self) -> f64 {
        to_encoded_double_raw(self)
    }
}

impl<T: IntoStahlVal> ToNanTaggedValue for T {
    fn encode(self) -> f64 {
        let value = self
            .into_stahlval()
            .expect("Value coercion to stahlval failed!");
...
// Take a pointer to a value that implements from stahl val and retrieve it
// Since these functions do coercion, this might be expensive and it may be better
// to move back and forth directly
// pub(crate) fn from_nan_tagged_value<T: FromStahlVal>(ptr: f64) -> T {
//     T::from_stahlval(&decode(ptr)).expect("Unable to coerce type from stahlval")
// }
...
pub fn to_encoded_double(value: &Gc<StahlVal>) -> f64 {
    match value.as_ref() {
        StahlVal::IntV(i) => from_i32(*i as i32),
        StahlVal::NumV(n) => *n,
        StahlVal::BoolV(b) => {
            if *b {
                to_float(TRUE_VALUE)
...
pub fn to_encoded_double_raw(value: &StahlVal) -> f64 {
    match value {
        StahlVal::IntV(i) => from_i32(*i as i32),
        StahlVal::NumV(n) => *n,
        StahlVal::BoolV(b) => {
        ...
        _ => coerce_value((value as *const StahlVal) as u64),
    }
}

pub fn to_encoded_double_raw_value(value: StahlVal) -> f64 {
    match value {
        StahlVal::IntV(i) => from_i32(i as i32),
        StahlVal::NumV(n) => n,
        StahlVal::BoolV(b) => {
...
pub fn to_encoded_double_from_const_ptr(value: *const StahlVal) -> f64 {
    coerce_value(value as u64)
}

pub fn decode(ptr: f64) -> StahlVal {
    let bits = ptr.to_bits();
    if is_int32(bits) {
        // println!("Decoding an int");
        StahlVal::IntV(get_int32(ptr) as isize)
    } else if is_double(bits) {
        // println!("Decoding a double");
        StahlVal::NumV(ptr)
    } else if is_bool(bits) {
        StahlVal::BoolV(get_as_rust_bool(bits))
    } else if is_string(bits) {
        // println!("Decoding a stahlval ref");
...
pub unsafe fn get_ref_from_double(ptr: f64) -> StahlVal {
    (&*(get_pointer(ptr) as *const StahlVal)).clone()
}

// This is super duper spooky
pub unsafe fn get_value_from_double(ptr: f64) -> StahlVal {
    StahlVal::StringV(std::rc::Rc::from_raw(get_pointer(ptr) as *const String).into())
}

pub unsafe fn get_string_from_double(ptr: f64) -> StahlVal {
    StahlVal::StringV(std::rc::Rc::from_raw(get_pointer(ptr) as *const String).into())
}
...
mod value_tests {
    ...
    //     let value = Gc::new(StahlVal::StringV("hello world".into()));
    //     let coerced = to_encoded_double(&value);
    //     let result = unsafe { get_ref_from_double(coerced) };
    //     assert_eq!(result, StahlVal::StringV("hello world".into()));
    // }

    // #[test]
    // fn test_pointer_conversion_raw() {
    //     let value = StahlVal::StringV("hello world".into());
    ...
    fn test_ints() {
        ...
        assert_eq!(StahlVal::IntV(1000), decoded);
    }
