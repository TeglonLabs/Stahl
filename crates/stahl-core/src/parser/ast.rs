use crate::{
    parser::{
        parser::SyntaxObject, tokens::TokenType::*, tryfrom_visitor::TryFromExprKindForStahlVal,
    },
    rvals::StahlComplex,
};
...
impl TryFrom<ExprKind> for StahlVal {
    type Error = StahlErr;

    fn try_from(e: ExprKind) -> std::result::Result<Self, Self::Error> {
        TryFromExprKindForStahlVal::try_from_expr_kind(e)
    }
}
...
#[derive(Debug)]
pub(crate) struct TryFromStahlValVisitorForExprKind {
    pub(crate) qq_depth: usize,
    pub(crate) quoted: bool,
}

impl TryFromStahlValVisitorForExprKind {
    pub fn root(value: &StahlVal) -> std::result::Result<ExprKind, StahlErr> {
        Self {
            qq_depth: 0,
            quoted: false,
        }
    ...
    pub fn visit(&mut self, value: &StahlVal) -> std::result::Result<ExprKind, StahlErr> {
        match value {
...
#[cold]
fn complex_to_literal(v: &StahlComplex) -> Result<NumberLiteral, &'static str> {
    let val_to_real = |real_val: &StahlVal| -> Result<RealLiteral, &'static str> {
        let l = match real_val {
            ...
            _ => return Err("StahlVal not a valid real number"),
        };
        Ok(l)
    };
...
impl TryFrom<&StahlVal> for ExprKind {
    type Error = &'static str;
    fn try_from(r: &StahlVal) -> std::result::Result<Self, Self::Error> {
        fn inner_try_from(
            r: &StahlVal,
            depth: usize,
        ) -> std::result::Result<ExprKind, &'static str> {
            if depth > 64 {
                return Err("Unable to convert stahl val to exprkind - depth was too large!");
            }
            ...
            match r {
                ...
                Boxed(_) => Err("Can't convert from boxed stahl val to expression!"),
