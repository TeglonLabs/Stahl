...
use stahl_parser::ast::{self, parse_lambda, Begin};
use stahl_parser::parser::SourceId;
use stahl_parser::tokens::NumberLiteral;

use crate::parser::ast::ExprKind;
use crate::parser::parser::SyntaxObject;
use crate::parser::span_visitor::get_span;
use crate::stahl_vm::engine::ModuleContainer;
...
pub fn extract_macro_defs(
    ...
    macro_map: &mut FxHashMap<InternedString, StahlMacro>,
) -> Result<()> {
    // let mut non_macros = Vec::new();
    // for expr in exprs {
    //     if let ExprKind::Macro(m) = expr {
    //         let generated_macro = StahlMacro::parse_from_ast_macro(m)?;
...
pub fn expand(expr: &mut ExprKind, map: &FxHashMap<InternedString, StahlMacro>) -> Result<()> {
...
pub fn expand_with_source_id(
    expr: &mut ExprKind,
    map: &FxHashMap<InternedString, StahlMacro>,
...
pub struct Expander<'a> {
    map: &'a FxHashMap<InternedString, StahlMacro>,
...
impl<'a> Expander<'a> {
    pub fn new(map: &'a FxHashMap<InternedString, StahlMacro>) -> Self {
        Self {
            map,
            changed: false,
            in_scope_values: ScopeSet::default(),
...
fn expr_usize(value: usize) -> ExprKind {
    ExprKind::Atom(Atom::new(SyntaxObject::default(TokenType::Number(
        Box::new(NumberLiteral::Real(stahl_parser::tokens::RealLiteral::Int(
            stahl_parser::tokens::IntLiteral::Small(value as _),
        ))),
    ))))
}
        ...
        .map(|(index, x)| match x {
            ...
            MaybeDefault::DefaultArg(p, default_value) => {
                ...
                let expression = expr_list![
                    ...
                    ExprKind::Atom(Atom::new(SyntaxObject::default(TokenType::Number(
                        Box::new(NumberLiteral::Real(stahl_parser::tokens::RealLiteral::Int(
                            stahl_parser::tokens::IntLiteral::Small(index as _)
                        )))
                    )))),
                    default_value.clone()
                ];
                        ...
                        if s == *REQUIRE_BUILTIN {
                            match &l.args[1..] {
                                [ExprKind::Atom(Atom {
                                    ...
                                    // (require-builtin stahl/obviouslydylib/sqlite (only-in ... ... ...)) <-
