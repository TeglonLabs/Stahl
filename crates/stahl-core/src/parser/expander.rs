        ...
        exprs
            ...
            .map(|x| {
                if let ExprKind::Macro(m) = x {
                    StahlMacro::parse_from_ast_macro(m)
                } else {
...
impl FromIterator<StahlMacro> for LocalMacroManager {
    fn from_iter<T>(iter: T) -> Self
    where
        T: IntoIterator<Item = StahlMacro>,
    {
        Self {
...
pub struct GlobalMacroManager {
    _scopes: HashMap<PathBuf, HashMap<String, StahlMacro>>,
}

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct StahlMacro {
    name: InternedString,
