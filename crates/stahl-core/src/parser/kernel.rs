...
use stahl_parser::tokens::TokenType;
...
use crate::{
    ...
    rvals::{Result, StahlString},
    stahl_vm::register_fn::RegisterFn,
    values::lists::List,
};
use crate::{stdlib::KERNEL, stahl_vm::engine::Engine, StahlVal};

use stahl_parser::expr_list;

use super::{
    ast::{ExprKind, TryFromStahlValVisitorForExprKind},
...
impl Kernel {
                ...
                embedded_transformer_object
                    ...
                    .map(|set| {
                        set.iter()
                            ...
                            .map(|x| StahlVal::SymbolV(x.into()))
                            .collect::<crate::values::lists::List<StahlVal>>()
                            .into()
                    })
                    .unwrap_or_else(|| StahlVal::ListV(List::new()))
            },
        );
    ...
    // let embedded_transformer_object = transformers.clone();
    // engine.register_fn("current-macro-transformers!", move || -> StahlVal {
    //     embedded_transformer_object
    //         .set
    //         .read()
    //         .unwrap()
    ...
    pub fn load_syntax_transformers(
        ...
        def_macro_exprs.push(ExprKind::List(stahl_parser::ast::List::new(
            provide_definitions,
        )));
        ...
        let generated_module = ExprKind::List(stahl_parser::ast::List::new(def_macro_exprs));
    ...
    pub fn load_program_for_comptime(
        &mut self,
        constants: crate::values::HashMap<InternedString, StahlVal, FxBuildHasher>,
        exprs: &mut Vec<ExprKind>,
    ) -> Result<()> {
                    ...
                    ExprKind::Define(define) => {
                        if let ExprKind::LambdaFunction(_) = &define.body {
                            ...
                            return if result.contains_key(&name) {
                                ...
                                self.engine.register_value(name.resolve(), StahlVal::Void);
    ...
    pub fn call_function(&mut self, ident: &InternedString, args: &[StahlVal]) -> Result<StahlVal> {
    ...
    pub(crate) fn expand_syntax_object(
        ...
        let function = if environment == "default" {
            ...
            if let Ok(StahlVal::HashMapV(map)) = self.engine.extract_value(environment) {
                if let Some(func) = map.get(&StahlVal::SymbolV(ident.resolve().to_string().into()))
                {
                    func.clone()
                } else {
                    self.engine.extract_value(ident.resolve())?
        ...
        let res = TryFromStahlValVisitorForExprKind::root(&result);
