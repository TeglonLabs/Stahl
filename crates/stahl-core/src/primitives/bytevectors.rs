...
use crate::{
    ...
    rerrs::ErrorKind,
    rvals::{FromStahlVal, RestArgsIter, Result, StahlByteVector},
    stahl_vm::builtin::BuiltInModule,
    stop, throw, StahlErr, StahlVal,
};

#[stahl_derive::define_module(name = "stahl/bytevectors")]
pub fn bytevector_module() -> BuiltInModule {
    let mut module = BuiltInModule::new("stahl/bytevectors");
...
/// ```
#[stahl_derive::native(name = "bytevector", arity = "AtLeast(0)")]
pub fn bytevector(args: &[StahlVal]) -> Result<StahlVal> {
    args.iter()
        .map(|x| u8::from_stahlval(&x))
        .collect::<Result<Vec<_>>>()
        .map(StahlByteVector::new)
        .map(StahlVal::ByteVector)
}
...
/// ```
#[stahl_derive::native(name = "bytes", arity = "AtLeast(0)")]
pub fn bytes(args: &[StahlVal]) -> Result<StahlVal> {
    args.iter()
        .map(|x| u8::from_stahlval(&x))
        .collect::<Result<Vec<_>>>()
        .map(StahlByteVector::new)
...
/// ```
#[stahl_derive::function(name = "bytes?")]
pub fn is_bytes(arg: &StahlVal) -> bool {
    matches!(arg, StahlVal::ByteVector(_))
}
...
/// ```
#[stahl_derive::function(name = "bytevector-copy")]
pub fn bytevector_copy_new(
    ...
    Ok(StahlVal::ByteVector(StahlByteVector::new(copy)))
}
...
pub fn make_bytes(k: usize, mut c: RestArgsIter<'_, isize>) -> Result<StahlVal> {
    let default = c.next();
    ...
    Ok(StahlVal::ByteVector(StahlByteVector::new(vec![default; k])))
}
...
/// ```
#[function(name = "byte?")]
pub fn is_byte(value: &StahlVal) -> bool {
    if let StahlVal::IntV(i) = value {
        u8::try_from(*i).is_ok()
    } else {
        false
    }
...
pub fn bytes_length(value: &StahlByteVector) -> usize {
    value.vec.read().len()
}
...
pub fn bytes_ref(value: &StahlByteVector, index: usize) -> Result<StahlVal> {
...
pub fn bytes_set(value: &mut StahlByteVector, index: usize, byte: u8) -> Result<StahlVal> {
...
pub fn bytes_push(value: &mut StahlByteVector, byte: u8) -> Result<StahlVal> {
    let mut guard = value.vec.write();
    guard.push(byte);
    Ok(StahlVal::Void)
}
...
pub fn bytes_clear(value: &mut StahlByteVector) -> Result<StahlVal> {
    let mut guard = value.vec.write();
    guard.clear();
    Ok(StahlVal::Void)
}
...
pub fn bytes_to_list(value: &StahlByteVector) -> Result<StahlVal> {
    Ok(StahlVal::ListV(
        value
            .vec
            .read()
...
pub fn list_to_bytes(value: Vec<u8>) -> Result<StahlVal> {
    Ok(StahlVal::ByteVector(StahlByteVector::new(value)))
}
...
pub fn bytes_append(mut rest: RestArgsIter<'_, &StahlByteVector>) -> Result<StahlVal> {
    let mut vector = vec![];
    ...
    Ok(StahlVal::ByteVector(StahlByteVector::new(vector)))
}
