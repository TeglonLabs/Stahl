use crate::rvals::{Custom, Result, StahlString, StahlVal};
use crate::stahl_vm::builtin::BuiltInModule;
use crate::{stahlerr, stop, throw};
use dirs;
...
#[stahl_derive::define_module(name = "stahl/filesystem")]
pub fn fs_module() -> BuiltInModule {
    let mut module = BuiltInModule::new("stahl/filesystem");
    module
...
pub fn fs_module_sandbox() -> BuiltInModule {
    BuiltInModule::new("stahl/filesystem")
}
...
/// ```
#[stahl_derive::function(name = "delete-directory!")]
pub fn delete_directory(directory: &StahlString) -> Result<StahlVal> {
...
/// ```
#[stahl_derive::function(name = "create-directory!")]
pub fn create_directory(directory: &StahlString) -> Result<StahlVal> {
...
/// ```
#[stahl_derive::function(name = "copy-directory-recursively!")]
pub fn copy_directory_recursively(
    source: &StahlString,
    destination: &StahlString,
) -> Result<StahlVal> {
...
/// ```
#[stahl_derive::function(name = "path-exists?")]
pub fn path_exists(path: &StahlString) -> Result<StahlVal> {
    Ok(StahlVal::BoolV(Path::new(path.as_ref()).exists()))
}
...
/// ```
#[stahl_derive::function(name = "is-file?")]
pub fn is_file(path: &StahlString) -> Result<StahlVal> {
    Ok(StahlVal::BoolV(Path::new(path.as_ref()).is_file()))
}
...
/// ```
#[stahl_derive::function(name = "is-dir?")]
pub fn is_dir(path: &StahlString) -> Result<StahlVal> {
    Ok(StahlVal::BoolV(Path::new(path.as_ref()).is_dir()))
}
...
/// ```
#[stahl_derive::function(name = "path->extension")]
pub fn get_extension(path: &StahlString) -> Result<StahlVal> {
    if let Some(ext) = get_extension_from_filename(path) {
        Ok(StahlVal::StringV(ext.into()))
    } else {
        Ok(StahlVal::Void)
    }
}
...
/// ```
#[stahl_derive::function(name = "file-name")]
pub fn file_name(path: &StahlString) -> Result<StahlVal> {
    Ok(StahlVal::StringV(
...
/// ```
#[stahl_derive::function(name = "parent-name")]
pub fn parent_name(path: &StahlString) -> Result<StahlVal> {
    Ok(StahlVal::StringV(
        Path::new(path.as_str())
            .parent()
            .and_then(|x| x.to_str())
...
/// ```
#[stahl_derive::function(name = "canonicalize-path")]
pub fn canonicalize_path(path: &StahlString) -> Result<StahlVal> {
    let path = path.as_str();
    let canonicalized = if path.len() > 0 && path.starts_with('~') {
        if path.len() > 1 && !path.starts_with("~/") {
            stahlerr!(Generic => "references to other users home directories are not supported")?
        } else {
...
/// > (read-dir "empty_dir") ;; => '()
/// ```
#[stahl_derive::function(name = "read-dir")]
pub fn read_dir(path: &StahlString) -> Result<StahlVal> {
    let p = Path::new(path.as_ref());
    if p.is_dir() {
        let iter = p.read_dir();
        match iter {
            Ok(i) => Ok(StahlVal::ListV(
                i.into_iter()
                    .map(|x| match x?.path().to_str() {
                        Some(s) => Ok(StahlVal::StringV(s.into())),
...
/// ```
#[stahl_derive::function(name = "current-directory")]
pub fn current_directory() -> Result<StahlVal> {
    let path = current_dir()?;
    Ok(StahlVal::StringV(path.to_str().unwrap_or("").into()))
}
...
/// ```
#[stahl_derive::function(name = "change-current-directory!")]
pub fn change_current_directory(path: &StahlString) -> Result<StahlVal> {
...
/// ```
#[stahl_derive::function(name = "delete-file!")]
pub fn delete_file(file: &StahlString) -> Result<StahlVal> {
