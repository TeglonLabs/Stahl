use crate::rvals::{IntoStahlVal, Result, StahlVal};
use crate::{
    gc::Gc,
    stahl_vm::{builtin::BuiltInModule, vm::APPLY_DEFINITION},
    values::lists::Pair,
};
...
declare_const_ref_functions! {
    LIST_TO_STRING => stahl_list_to_string,
}
...
pub struct UnRecoverableResult(Result<StahlVal>);

impl IntoStahlVal for UnRecoverableResult {
    #[inline(always)]
...
impl From<UnRecoverableResult> for Result<StahlVal> {
    fn from(value: UnRecoverableResult) -> Self {
        value.0
    }
}

impl From<Result<StahlVal>> for UnRecoverableResult {
    fn from(value: Result<StahlVal>) -> Self {
...
/// ```
#[stahl_derive::define_module(name = "stahl/lists")]
pub fn list_module() -> BuiltInModule {
    let mut module = BuiltInModule::new("stahl/lists");
    ...
    module
        ...
        .register_value("transduce", crate::stahl_vm::transducers::TRANSDUCE)
...
#[stahl_derive::function(name = "list-chunks", constant = true)]
pub fn chunks(list: &List<StahlVal>) -> Result<StahlVal> {
    let nodes = list.nodes();

    Ok(StahlVal::ListV(
        nodes.into_iter().map(|x| StahlVal::ListV(x)).collect(),
    ))
}
...
#[stahl_derive::function(name = "second", constant = true)]
pub fn second(list: &List<StahlVal>) -> Result<StahlVal> {
...
/// ```
#[stahl_derive::function(name = "third", constant = true)]
pub(crate) fn third(list: &List<StahlVal>) -> Result<StahlVal> {
...
#[stahl_derive::function(name = "list-tail")]
pub fn list_tail(list_or_pair: &StahlVal, pos: usize) -> Result<StahlVal> {
    match list_or_pair {
        StahlVal::ListV(l) => l
            .tail(pos)
...
/// ```
#[stahl_derive::native(name = "list", arity = "AtLeast(0)")]
pub fn new(args: &[StahlVal]) -> Result<StahlVal> {
    Ok(StahlVal::ListV(args.iter().cloned().collect()))
}
...
/// > (empty? '()) ;; => #true
/// ```
#[stahl_derive::function(name = "empty?")]
pub fn is_empty(list: &StahlVal) -> bool {
...
/// > (pair? '()) ;; => #false
/// ```
#[stahl_derive::function(name = "pair?")]
fn pair(list: &StahlVal) -> bool {
    match list {
        StahlVal::ListV(l) => !l.is_empty(),
        StahlVal::Pair(_) => true,
        _ => false,
    }
}
...
/// > (cons 1 '()) ;; => '(1)
/// ```
#[stahl_derive::function(name = "cons", arity = "Exact(2)")]
pub fn cons(arg: &mut StahlVal, arg2: &mut StahlVal) -> Result<StahlVal> {
    match (std::mem::replace(arg, StahlVal::Void), arg2) {
        (left, StahlVal::ListV(right)) => {
            right.cons_mut(left);

            // Consider moving in a default value instead of cloning?
            Ok(StahlVal::ListV(right.clone()))
        }
        // Silly, but this then gives us a special "pair" that is different
        // from a real bonafide list
        (left, right) => Ok(StahlVal::Pair(Gc::new(Pair::cons(left, right.clone())))),
    }
}
...
// pub(crate) fn unsafe_cons(args: &mut [StahlVal]) -> Result<StahlVal> {
...
/// ```
#[stahl_derive::function(name = "range")]
fn range(lower: isize, upper: isize) -> Result<StahlVal> {
    if lower < 0 {
...
/// ```
#[stahl_derive::function(name = "length")]
fn length(list: &List<StahlVal>) -> usize {
    list.len()
}
...
/// ```
#[stahl_derive::function(name = "reverse", constant = true)]
fn reverse(arg: &mut StahlVal) -> Result<StahlVal> {
    if let StahlVal::ListV(l) = std::mem::replace(arg, StahlVal::Void) {
        Ok(StahlVal::ListV(l.reverse()))
    } else {
...
/// > (list (list 1 2 3 4)) ;; => 4
/// ```
#[stahl_derive::function(name = "last", constant = true)]
pub fn last(list: &List<StahlVal>) -> Result<StahlVal> {
    list.last()
        .cloned()
...
/// > (first (cons 2 3)) ;; => 2
/// ```
#[stahl_derive::function(name = "first", constant = true)]
fn first(list: &List<StahlVal>) -> Result<StahlVal> {
    list.car()
...
/// > (car (cons 2 3)) ;; => 2
/// ```
#[stahl_derive::function(name = "car", constant = true)]
pub(crate) fn car(list: &StahlVal) -> Result<StahlVal> {
    match list {
        StahlVal::ListV(l) => l
            .car()
            .ok_or_else(throw!(Generic => "car resulted in an error - empty list")),

        StahlVal::Pair(p) => Ok(p.car()),
...
#[stahl_derive::native(name = "cdr-null?", constant = true, arity = "Exact(1)")]
fn cdr_is_null(args: &[StahlVal]) -> Result<StahlVal> {
    ...
    match &args[0] {
        StahlVal::ListV(l) => {
            ...
            Ok(StahlVal::BoolV(!l.cdr_exists()))
        }
        other => {
...
/// ```
#[stahl_derive::function(name = "cdr", constant = true)]
pub(crate) fn cdr(arg: &mut StahlVal) -> Result<StahlVal> {
    match std::mem::replace(arg, StahlVal::Void) {
        StahlVal::ListV(mut l) => {
            if l.is_empty() {
                stop!(Generic => "cdr expects a non empty list");
            }

            match l.rest_mut() {
                Some(_) => Ok(StahlVal::ListV(l)),
                None => Ok(StahlVal::ListV(l)),
            }
        }

        StahlVal::Pair(p) => Ok(p.cdr()),
        arg => {
...
/// ```
#[stahl_derive::function(name = "rest", constant = true, arity = "Exact(1)")]
fn rest(arg: &mut StahlVal) -> Result<StahlVal> {
    if let StahlVal::ListV(mut l) = std::mem::replace(arg, StahlVal::Void) {
        if l.is_empty() {
            stop!(Generic => "rest expects a non empty list");
        }

        match l.rest_mut() {
            Some(l) => Ok(StahlVal::ListV(l.clone())),
            None => Ok(StahlVal::ListV(l.clone())),
        }
    } else {
...
/// ```
#[stahl_derive::function(name = "take", constant = true)]
fn take(list: &List<StahlVal>, n: isize) -> Result<StahlVal> {
    if n < 0 {
        stop!(Generic => "take expects a positive integer")
    } else {
        Ok(StahlVal::ListV(list.take(n as usize)))
    }
}
...
/// > (append) ;; => '()
/// ```
#[stahl_derive::native_mut(name = "append", constant = true, arity = "AtLeast(0)")]
fn append(args: &mut [StahlVal]) -> Result<StahlVal> {
    if let Some((first, rest)) = args.split_first_mut() {
        let initial = if let StahlVal::ListV(ref mut l) = first {
            l
        } else {
        ...
        return Ok(StahlVal::ListV(initial.clone()));
    } else {
        return Ok(StahlVal::ListV(List::new()));
    }
}

#[stahl_derive::function(name = "try-list-ref", constant = true)]
pub fn try_list_ref(list: &List<StahlVal>, index: isize) -> Result<StahlVal> {
    if index < 0 {
...
#[stahl_derive::function(name = "list-drop", constant = true)]
pub fn drop_start(list: &List<StahlVal>, skip: usize) -> Result<StahlVal> {
    let mut list = list.clone();
    for _ in 0..skip {
        list.cdr_mut();
    }
    Ok(StahlVal::ListV(list))
}

#[stahl_derive::function(name = "plist-get", constant = true)]
pub fn plist_get(list: &List<StahlVal>, key: &StahlVal) -> Result<StahlVal> {
...
#[stahl_derive::function(name = "plist-get-positional-arg-list")]
pub fn plist_get_positional_list(list: &List<StahlVal>, index: usize) -> Result<StahlVal> {
    let mut iter = list.iter();
    let mut positional_arg_offset = 0;

    while let Some(next) = iter.next() {
        if let StahlVal::SymbolV(v) = next {
        ...
        if index == positional_arg_offset {
            ...
            return Ok(StahlVal::ListV(list));
        }

        positional_arg_offset += 1;
    }

    Ok(StahlVal::ListV(List::new()))
}

#[stahl_derive::function(name = "plist-validate-args")]
...
// Find the arg by index, skipping keyword pairs
#[stahl_derive::function(name = "plist-get-positional-arg")]
pub fn plist_get_positional(list: &List<StahlVal>, index: usize) -> Result<StahlVal> {
    let mut iter = list.iter();
    let mut positional_arg_offset = 0;
...
// Find the arg by index, skipping keyword pairs
#[stahl_derive::function(name = "plist-try-get-positional-arg")]
pub fn plist_try_get_positional(
    list: &List<StahlVal>,
    index: usize,
    default_value: StahlVal,
) -> Result<StahlVal> {
    let mut iter = list.iter();
    let mut positional_arg_offset = 0;
...
#[stahl_derive::function(name = "plist-get-kwarg", constant = true)]
...
#[stahl_derive::function(name = "plist-try-get", constant = true)]
...
/// ```
#[stahl_derive::function(name = "list-ref", constant = true)]
pub fn list_ref(list: &List<StahlVal>, index: isize) -> Result<StahlVal> {
    if index < 0 {
...
#[stahl_derive::function(name = "list->string", constant = true)]
fn list_to_string(list: &List<StahlVal>) -> Result<StahlVal> {
    list.iter()
        .map(|x| {
...
#[stahl_derive::function(name = "list->vector")]
fn list_to_vector(list: &List<StahlVal>) -> StahlVal {
    let args: crate::values::Vector<_> = list.iter().cloned().collect();

    StahlVal::VectorV(Gc::new(args).into())
}

// TODO this could be broken using &mut
// need to think about it more
fn push_back(args: &mut [StahlVal]) -> Result<StahlVal> {
    arity_check!(push_back, args, 2);

    let arg = args[1].clone();

    if let StahlVal::ListV(l) = &mut args[0] {
        l.push_back(arg);
...
impl From<List<StahlVal>> for StahlVal {
    fn from(l: List<StahlVal>) -> Self {
        StahlVal::ListV(l)
    }
}
...
mod list_operation_tests {
    ...
    fn cons_test_normal_input() {
        let mut args = [StahlVal::IntV(1), StahlVal::IntV(2)];
        let res = stahl_cons(&mut args);

        let expected = StahlVal::Pair(Gc::new(Pair::cons(StahlVal::IntV(1), StahlVal::IntV(2))));
    ...
    fn cons_with_empty_list() {
        let mut args = [StahlVal::IntV(1), StahlVal::ListV(List::new())];
        let res = stahl_cons(&mut args);
    ...
    fn cons_with_non_empty_vector() {
        let mut args = [
            StahlVal::IntV(1),
            StahlVal::ListV(vec![StahlVal::IntV(2)].into()),
        ];
        let res = stahl_cons(&mut args);
    ...
    fn car_normal_input() {
        ...
        let res = stahl_car(&args);
        let expected = StahlVal::IntV(1);
        assert_eq!(res.unwrap(), expected);
    }

    #[test]
    fn car_bad_input() {
        let args = [StahlVal::IntV(1)];
        let res = stahl_car(&args);
    ...
    fn car_too_many_args() {
        let args = [StahlVal::IntV(1), StahlVal::IntV(2)];
        let res = stahl_car(&args);
        let expected = ErrorKind::ArityMismatch;
        assert_eq!(res.unwrap_err().kind(), expected);
    }
    ...
    fn cdr_too_many_args() {
        let mut args = [StahlVal::NumV(1.0), StahlVal::NumV(2.0)];
        let res = stahl_cdr(&mut args);
        let expected = ErrorKind::ArityMismatch;
        assert_eq!(res.unwrap_err().kind(), expected);
    }
    ...
    fn cdr_single_element_list() {
        let mut args = [StahlVal::ListV(vec![StahlVal::NumV(1.0)].into())];
        let res = stahl_cdr(&mut args);
    ...
    fn range_test_normal_input() {
        let args = [StahlVal::IntV(0), StahlVal::IntV(3)];
        let res = stahl_range(&args);
        let expected =
            StahlVal::ListV(vec![StahlVal::IntV(0), StahlVal::IntV(1), StahlVal::IntV(2)].into());
        assert_eq!(res.unwrap(), expected);
    }
}
