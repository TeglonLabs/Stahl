...
use crate::stop;
use crate::values::port::{would_block, StahlPort, StahlPortRepr, WOULD_BLOCK_OBJECT};
...
thread_local! {
    static EOF_OBJECT: once_cell::unsync::Lazy<(StahlVal, StructTypeDescriptor)>= once_cell::unsync::Lazy::new(|| {
        make_struct_singleton("eof".into())
    });
}

#[cfg(feature = "sync")]
pub static EOF_OBJECT: once_cell::sync::Lazy<(StahlVal, StructTypeDescriptor)> =
    once_cell::sync::Lazy::new(|| make_struct_singleton("eof".into()));

pub fn port_module() -> BuiltInModule {
    let mut module = BuiltInModule::new("stahl/ports");
    module
...
/// ```
#[function(name = "stdin")]
pub fn open_stdin() -> StahlVal {
    StahlVal::PortV(StahlPort {
        port: Gc::new_mut(StahlPortRepr::StdInput(std::io::stdin())),
    })
}

#[function(name = "stdout")]
pub fn open_stdout() -> StahlVal {
    StahlVal::PortV(StahlPort {
        port: Gc::new_mut(StahlPortRepr::StdOutput(std::io::stdout())),
    })
}
...
pub fn open_input_file(path: &StahlString) -> Result<StahlVal> {
    StahlPort::new_textual_file_input(path).map(StahlVal::PortV)
}
...
pub fn open_output_file(path: &StahlString) -> Result<StahlVal> {
    StahlPort::new_textual_file_output(path).map(StahlVal::PortV)
}
...
pub fn open_output_string() -> StahlVal {
    StahlVal::PortV(StahlPort::new_output_port_string())
}
...
pub fn open_output_bytevector() -> StahlVal {
    StahlVal::PortV(StahlPort::new_output_port_string())
}
...
pub fn open_input_string(s: &StahlString) -> StahlVal {
    StahlVal::PortV(StahlPort::new_input_port_string(s.to_string()))
}
...
pub fn open_input_bytevector(bytes: &StahlByteVector) -> StahlVal {
    let vec: &Vec<u8> = &*bytes.vec.read();
    StahlVal::PortV(StahlPort::new_input_port_bytevector(vec.clone()))
}
...
pub fn read_port_to_string(port: &StahlPort) -> Result<StahlVal> {
    let (_, result) = port.read_all_str()?;
    Ok(StahlVal::StringV(result.into()))
}
...
pub fn is_input(maybe_port: &StahlVal) -> bool {
    if let StahlVal::PortV(port) = maybe_port {
        port.is_input()
    } else {
        false
    }
...
pub fn is_output(maybe_port: &StahlVal) -> bool {
    if let StahlVal::PortV(port) = maybe_port {
        port.is_output()
    } else {
        false
...
pub fn read_line_to_string(port: &StahlPort) -> Result<StahlVal> {
...
pub fn write_line(port: &StahlPort, line: &StahlVal) -> Result<StahlVal> {
...
pub fn write(line: &StahlVal, rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
...
pub fn write_char(character: char, rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = output_args(rest)?;
...
pub fn write_string(line: &StahlVal, rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = output_args(rest)?;

    let res = if let StahlVal::StringV(s) = line {
...
pub fn get_output_string(port: &StahlPort) -> Result<StahlVal> {
...
pub fn get_output_bytevector(port: &StahlPort) -> Result<StahlVal> {
    let Some(bytes) = port.get_output()? else {
...
pub fn flush_output_port(port: &StahlPort) -> Result<StahlVal> {
    port.flush().map(|_| StahlVal::Void)
}

#[function(name = "#%default-input-port")]
pub fn default_input_port() -> StahlVal {
    StahlVal::PortV(StahlPort::default_current_input_port())
}

#[function(name = "#%default-output-port")]
pub fn default_output_port() -> StahlVal {
    StahlVal::PortV(StahlPort::default_current_output_port())
}

#[function(name = "#%default-error-port")]
pub fn default_error_port() -> StahlVal {
    StahlVal::PortV(StahlPort::default_current_error_port())
}

#[function(name = "close-output-port")]
pub fn close_output_port(port: &StahlPort) -> Result<StahlVal> {
    port.close_output_port().map(|_| StahlVal::Void)
}

#[function(name = "close-input-port")]
pub fn close_input_port(port: &StahlPort) -> Result<StahlVal> {
    port.close_input_port().map(|_| StahlVal::Void)
}
...
pub fn eof_objectp(value: &StahlVal) -> bool {
...
#[function(name = "eof-object")]
pub fn eof_object() -> StahlVal {
    eof()
}

#[function(name = "would-block")]
pub fn would_block_object() -> StahlVal {
    would_block()
}
...
pub fn would_block_objectp(value: &StahlVal) -> bool {
    let StahlVal::CustomStruct(struct_) = value else {
        return false;
    };
...
pub fn read_byte(rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    ...
    match maybe_byte {
        crate::values::port::MaybeBlocking::Nonblocking(b) => {
            Ok(b.map(|b| StahlVal::IntV(b.into())).unwrap_or_else(eof))
        }
...
pub fn read_bytes(amt: usize, rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = input_args(rest)?;
    ...
    match bytes {
        crate::values::port::MaybeBlocking::Nonblocking(b) => {
            Ok(StahlVal::ByteVector(StahlByteVector::new(b).into()))
        }
        crate::values::port::MaybeBlocking::WouldBlock => Ok(would_block_object()),
    }
}
...
pub fn write_byte(byte: u8, rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = output_args(rest)?;
    port.write(&[byte])?;
...
pub fn write_bytes(bytes: &StahlByteVector, rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = output_args(rest)?;
...
pub fn peek_byte(rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = input_args(rest)?;
...
pub fn read_char(rest: RestArgsIter<&StahlPort>) -> Result<StahlVal> {
    let port = input_args(rest)?;
    ...
    match char {
        crate::values::port::MaybeBlocking::Nonblocking(c) => {
            Ok(c.map(StahlVal::CharV).unwrap_or_else(eof))
        }
        crate::values::port::MaybeBlocking::WouldBlock => Ok(would_block_object()),
    }
}

fn input_args(args: RestArgsIter<&StahlPort>) -> Result<StahlPort> {
    Ok(io_args(1, args)?.unwrap_or_else(StahlPort::default_current_input_port))
}

fn output_args(args: RestArgsIter<&StahlPort>) -> Result<StahlPort> {
    Ok(io_args(2, args)?.unwrap_or_else(StahlPort::default_current_output_port))
}

fn io_args(max: usize, mut args: RestArgsIter<&StahlPort>) -> Result<Option<StahlPort>> {
