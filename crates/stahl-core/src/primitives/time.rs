use crate::gc::Gc;
use crate::StahlVal;
use crate::{rvals::Custom, stahl_vm::builtin::MarkdownDoc};
...
fn current_milliseconds() -> StahlVal {
    ...
    match SystemTime::now().duration_since(UNIX_EPOCH) {
        Ok(n) => {
            ...
            match isize::try_from(ms) {
                Ok(inner) => StahlVal::IntV(inner),
                _ => StahlVal::BigNum(Gc::new(num::BigInt::from(ms))),
            }
        }
...
pub fn time_module() -> BuiltInModule {
    let mut module = BuiltInModule::new("stahl/time".to_string());

    module.register_doc("stahl/time", TIME_MODULE_DOC);
