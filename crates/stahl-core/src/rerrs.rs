use crate::stahl_vm::vm::DehydratedCallContext;
use crate::{parser::parser::ParseError, rvals::Custom, stahl_vm::vm::DehydratedStackTrace};
...
use stahl_parser::parser::SourceId;
...
impl From<std::io::Error> for StahlErr {
    fn from(v: std::io::Error) -> Self {
        StahlErr::_new(v.into())
    }
}
...
impl From<Infallible> for StahlErr {
    fn from(v: Infallible) -> Self {
        StahlErr::_new(v.into())
    }
}

impl From<ParseError> for StahlErr {
...
impl std::error::Error for StahlErr {}

impl fmt::Display for StahlErr {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.repr)
    }
}

impl Custom for StahlErr {
...
#[macro_export]
macro_rules! stahlerr {
    // ($type:ident) => {
    //     return Err(StahlErr::new(ErrorKind::$type, None));
    // };
    ($type:ident => $fmt:expr, $($arg:tt)+) => {
        Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, format!($fmt, $($arg)+)))
    };
    ($type:ident => $thing:expr) => {
        Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()))
    };
    ($type:ident => $thing:expr; $span:expr) => {
        Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()).with_span($span))
    };
    ($type:ident => $thing:expr; $span:expr; $source:expr) => {
        Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()).with_span($span).with_source($source))
    };
}
...
macro_rules! stop {
    ...
    ($type:ident => $fmt:expr, $($arg:tt),+ ; $span:expr) => {
        return Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, format!($fmt, $($arg),+)).with_span($span))
    };
    ($type:ident => $fmt:expr, $($arg:tt)+) => {
        return Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, format!($fmt, $($arg)+)))
    };
    ($type:ident => $thing:expr) => {
        return Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()))
    };
    ($type:ident => $thing:expr; $span:expr) => {
        return Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()).with_span($span))
    };
...
macro_rules! builtin_stop {
    ...
    // };
    ($type:ident => $fmt:expr, $($arg:tt)+) => {
        return Some(Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, format!($fmt, $($arg)+))))
    };
    ($type:ident => $thing:expr) => {
        return Some(Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string())))
    };
    ($type:ident => $thing:expr; $span:expr) => {
        return Some(Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()).with_span($span)))
    };
    ($type:ident => $thing:expr; $span:expr; $source:expr) => {
        return Some(Err($crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()).with_span($span).with_source($source)))
    };
}
...
macro_rules! throw {
    ...
    // };
    ($type:ident => $fmt:expr, $($arg:tt)+) => {
        || $crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, format!($fmt, $($arg)+))
    };
    ($type:ident => $thing:expr) => {
        || $crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string())
    };
    ($type:ident => $thing:expr; $span:expr) => {
        || $crate::rerrs::StahlErr::new($crate::rerrs::ErrorKind::$type, ($thing).to_string()).with_span($span)
    };
}
