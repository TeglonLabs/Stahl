...
use crate::stahl_vm::{builtin::get_function_name, vm::Continuation, vm::ContinuationMark};
...
impl CycleDetector {
    pub(super) fn detect_and_display_cycles(val: &StahlVal, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    fn start_format(mut self, val: &StahlVal, f: &mut fmt::Formatter) -> fmt::Result {
        for node in std::mem::take(&mut self.values) {
            let id = match &node {
                StahlVal::CustomStruct(c) => {
                    let ptr_addr = c.as_ptr() as usize;
                    self.cycles.get(&ptr_addr).unwrap()
                }
                StahlVal::HeapAllocated(b) => {
            ...
            CustomStruct(s) => match format_type {
                FormatType::Normal => {
                    if let Some(id) = self.cycles.get(&(s.as_ptr() as usize)) {
                        ...
                        {
                            if s.get(&StahlVal::SymbolV(StahlString::from("#:transparent")))
...
fn replace_with_void(value: &mut StahlVal) -> StahlVal {
    std::mem::replace(value, StahlVal::Void)
}

impl StahlVal {
    fn make_void(&mut self) -> StahlVal {
        std::mem::replace(self, StahlVal::Void)
    }
}

pub(crate) struct StahlCycleCollector {
    cycles: fxhash::FxHashMap<usize, usize>,
    values: List<StahlVal>,
}

impl Custom for StahlCycleCollector {}

impl StahlCycleCollector {
    pub fn from_root(value: StahlVal) -> Self {
        let mut queue = Vec::new();
    ...
    // Get the value
    pub fn get(&self, node: StahlVal) -> Option<usize> {
        match node {
            StahlVal::CustomStruct(c) => {
                let ptr_addr = c.as_ptr() as usize;
    ...
    pub fn values(&self) -> List<StahlVal> {
        self.values.clone()
    }
}
...
impl<'a> CycleCollector<'a> {
    fn add(&mut self, val: usize, stahlval: &StahlVal) -> bool {
        ...
        if self.visited.contains(&val) {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = self.cycles.entry(val) {
                ...
                self.values.push(stahlval.clone());
            } else {
                return true;
            }
...
impl<'a> BreadthFirstSearchStahlValVisitor for CycleCollector<'a> {
    type Output = ();
    ...
    fn visit_bytevector(&mut self, _bytevector: StahlByteVector) -> Self::Output {}
    ...
    fn visit_hash_map(&mut self, hashmap: StahlHashMap) -> Self::Output {
        if !self.add(
            ...
            &StahlVal::HashMapV(hashmap.clone()),
        ) {
    ...
    fn visit_hash_set(&mut self, hashset: StahlHashSet) -> Self::Output {
        if !self.add(
            ...
            &StahlVal::HashSetV(hashset.clone()),
        ) {
    ...
    fn visit_stahl_struct(&mut self, stahl_struct: Gc<UserDefinedStruct>) -> Self::Output {
        if !self.add(
            stahl_struct.as_ptr() as usize,
            &StahlVal::CustomStruct(stahl_struct.clone()),
        ) {
            for value in stahl_struct.fields.iter() {
                self.push_back(value.clone())
            }
        }
    }
    ...
    fn visit_list(&mut self, list: List<StahlVal>) -> Self::Output {
        if !self.add(list.as_ptr_usize(), &StahlVal::ListV(list.clone())) {
            for value in list {
    ...
    fn visit_mutable_vector(&mut self, vector: HeapRef<Vec<StahlVal>>) -> Self::Output {
        ...
        if !self.add(
            vector.as_ptr_usize(),
            &StahlVal::MutableVector(vector.clone()),
        ) {
    ...
    fn visit_syntax_object(&mut self, syntax_object: Gc<Syntax>) -> Self::Output {
        if !self.add(
            ...
            &StahlVal::SyntaxObject(syntax_object.clone()),
        ) {
    ...
    fn visit_boxed_value(&mut self, boxed_value: GcMut<StahlVal>) -> Self::Output {
        if !self.add(
            ...
            &StahlVal::Boxed(boxed_value.clone()),
        ) {
    ...
    fn visit_heap_allocated(&mut self, heap_ref: HeapRef<StahlVal>) -> Self::Output {
        ...
        if !self.add(
            ...
            &StahlVal::HeapAllocated(heap_ref.clone()),
        ) {
...
pub(crate) mod drop_impls {
    ...
    thread_local! {
        pub static DROP_BUFFER: RefCell<VecDeque<StahlVal>> = RefCell::new(VecDeque::with_capacity(128));
        pub static FORMAT_BUFFER: RefCell<VecDeque<StahlVal>> = RefCell::new(VecDeque::with_capacity(128));
    }

    impl Drop for StahlVector {
        fn drop(&mut self) {
            if self.0.is_empty() {
                return;
            }
    ...
    impl Drop for LazyStream {
        fn drop(&mut self) {
            if self.initial_value == StahlVal::Void && self.stream_thunk == StahlVal::Void {
                return;
            }
...
pub struct IterativeDropHandler<'a> {
    drop_buffer: &'a mut VecDeque<StahlVal>,
    #[cfg(feature = "experimental-drop-handler")]
    moved_threads: bool,
}

impl<'a> IterativeDropHandler<'a> {
    pub fn bfs(drop_buffer: &'a mut VecDeque<StahlVal>) {
...
impl<'a> BreadthFirstSearchStahlValVisitor for IterativeDropHandler<'a> {
    type Output = ();
    ...
    fn visit_immutable_vector(&mut self, mut vector: StahlVector) {
    ...
    fn visit_hash_map(&mut self, mut hashmap: StahlHashMap) {
    ...
    fn visit_hash_set(&mut self, mut hashset: StahlHashSet) {
    ...
    fn visit_stahl_struct(&mut self, stahl_struct: Gc<UserDefinedStruct>) {
        if let Ok(mut inner) = stahl_struct.try_unwrap() {
    ...
    fn visit_list(&mut self, list: List<StahlVal>) {
    ...
    // effectively!
    fn visit_mutable_vector(&mut self, _vector: HeapRef<Vec<StahlVal>>) {}
    ...
    fn visit_boxed_value(&mut self, boxed_value: GcMut<StahlVal>) {
    ...
    fn visit_heap_allocated(&mut self, _heap_ref: HeapRef<StahlVal>) -> Self::Output {}
...
pub struct OwnedIterativeDropHandler {
    drop_buffer: VecDeque<StahlVal>,
}

#[cfg(feature = "experimental-drop-handler")]
impl BreadthFirstSearchStahlValVisitor for OwnedIterativeDropHandler {
    type Output = ();
...
pub trait BreadthFirstSearchStahlValVisitor {
    type Output;
    ...
    fn visit_immutable_vector(&mut self, vector: StahlVector) -> Self::Output;
    fn visit_void(&mut self) -> Self::Output;
    fn visit_string(&mut self, string: StahlString) -> Self::Output;
    fn visit_function_pointer(&mut self, ptr: FunctionSignature) -> Self::Output;
    fn visit_symbol(&mut self, symbol: StahlString) -> Self::Output;
    fn visit_custom_type(&mut self, custom_type: GcMut<Box<dyn CustomType>>) -> Self::Output;
    fn visit_hash_map(&mut self, hashmap: StahlHashMap) -> Self::Output;
    fn visit_hash_set(&mut self, hashset: StahlHashSet) -> Self::Output;
    fn visit_stahl_struct(&mut self, stahl_struct: Gc<UserDefinedStruct>) -> Self::Output;
    fn visit_port(&mut self, port: StahlPort) -> Self::Output;
    ...
    fn visit_heap_allocated(&mut self, heap_ref: HeapRef<StahlVal>) -> Self::Output;
    fn visit_pair(&mut self, pair: Gc<Pair>) -> Self::Output;
    fn visit_bytevector(&mut self, bytevector: StahlByteVector) -> Self::Output;
}

pub trait BreadthFirstSearchStahlValReferenceVisitor<'a> {
    type Output;

    fn default_output(&mut self) -> Self::Output;

    fn pop_front(&mut self) -> Option<&'a StahlVal>;

    fn push_back(&mut self, value: &'a StahlVal);
    ...
    fn visit_bytevector(&mut self, _: &'a StahlByteVector) -> Self::Output;
    ...
    fn visit_char(&mut self, _: char) -> Self::Output;
    fn visit_immutable_vector(&mut self, vector: &'a StahlVector) -> Self::Output;
    fn visit_void(&mut self) -> Self::Output;
    fn visit_string(&mut self, string: &'a StahlString) -> Self::Output;
    fn visit_function_pointer(&mut self, ptr: FunctionSignature) -> Self::Output;
    fn visit_symbol(&mut self, symbol: &'a StahlString) -> Self::Output;
    fn visit_custom_type(&mut self, custom_type: &'a GcMut<Box<dyn CustomType>>) -> Self::Output;
    fn visit_hash_map(&mut self, hashmap: &'a StahlHashMap) -> Self::Output;
    fn visit_hash_set(&mut self, hashset: &'a StahlHashSet) -> Self::Output;
    fn visit_stahl_struct(&mut self, stahl_struct: &'a Gc<UserDefinedStruct>) -> Self::Output;
    fn visit_port(&mut self, port: &'a StahlPort) -> Self::Output;
    ...
    fn visit_list(&mut self, list: &'a List<StahlVal>) -> Self::Output;
    fn visit_mutable_function(&mut self, function: &'a MutFunctionSignature) -> Self::Output;
    fn visit_mutable_vector(&mut self, vector: &'a HeapRef<Vec<StahlVal>>) -> Self::Output;
    ...
    fn visit_boxed_value(&mut self, boxed_value: &'a GcMut<StahlVal>) -> Self::Output;
    fn visit_reference_value(
        &mut self,
        reference: &'a Gc<OpaqueReference<'static>>,
    ) -> Self::Output;
    fn visit_heap_allocated(&mut self, heap_ref: &'a HeapRef<StahlVal>) -> Self::Output;
    fn visit_pair(&mut self, pair: &'a Gc<Pair>) -> Self::Output;
}

thread_local! {
    static LEFT_QUEUE: RefCell<Vec<StahlVal>> = RefCell::new(Vec::with_capacity(128));
    static RIGHT_QUEUE: RefCell<Vec<StahlVal>> = RefCell::new(Vec::with_capacity(128));
...
impl<'a> RecursiveEqualityHandler<'a> {
    pub fn compare_equality(&mut self, left: StahlVal, right: StahlVal) -> bool {
    ...
    fn visit(&mut self) -> bool {
        loop {
            ...
            match (left, right) {
                (ListV(l), ListV(r)) => {
                        ...
                        && self.should_visit(r.elements_as_ptr_usize())
                    {
                        ...
                        for (lvalue, rvalue) in l.iter().zip(r.iter()) {
                            ...
                            match (lvalue, rvalue) {
                                (StahlVal::ListV(llist), StahlVal::ListV(rlist))
                                ...
                                // (StahlVal::ListV(llist), StahlVal::ListV(rlist)) if llist.len() == 1 && rlist.len() == 1 {
                ...
                (StahlVal::Custom(l), StahlVal::Custom(r)) => {
                ...
                (StahlVal::Custom(l), other) => {
                    if l.read().check_equality_hint_general(&other) {
                        ...
                        match other {
                            ...
                            StahlVal::Custom(r) => self.right.visit_custom_type(r),
                            HashMapV(r) => self.right.visit_hash_map(r),
                            HashSetV(r) => self.right.visit_hash_set(r),
                            CustomStruct(r) => self.right.visit_stahl_struct(r),
                ...
                (other, StahlVal::Custom(r)) => {
                    if r.read().check_equality_hint_general(&other) {
                        match other {
                            ...
                            StahlVal::Custom(r) => self.left.visit_custom_type(r),
                            HashMapV(r) => self.left.visit_hash_map(r),
                            HashSetV(r) => self.left.visit_hash_set(r),
                            CustomStruct(r) => self.left.visit_stahl_struct(r),
                ...
                (StahlVal::HashMapV(l), StahlVal::HashMapV(r)) => {
                    if Gc::ptr_eq(&l.0, &r.0) {
                        continue;
                    }
                ...
                (CustomStruct(l), CustomStruct(r)) => {
                        ...
                        && self.should_visit(r.as_ptr() as usize)
                    {
                        ...
                        self.left.visit_stahl_struct(l);
                        self.right.visit_stahl_struct(r);
                    }
...
impl<'a> BreadthFirstSearchStahlValVisitor for EqualityVisitor<'a> {
    type Output = ();
    ...
    fn visit_hash_map(&mut self, _hashmap: StahlHashMap) -> Self::Output {
        // TODO: See comment above
    }

    fn visit_hash_set(&mut self, _hashset: StahlHashSet) -> Self::Output {
        // TODO: See comment above
    }

    fn visit_stahl_struct(&mut self, stahl_struct: Gc<UserDefinedStruct>) -> Self::Output {
        // if self.should_visit(stahl_struct.as_ptr() as usize) {
        for value in stahl_struct.fields.iter() {
            self.push_back(value.clone());
        }
        // }
