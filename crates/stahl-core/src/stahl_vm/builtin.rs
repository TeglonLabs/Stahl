...
use crate::{
    ...
    rvals::{
        Custom, FromStahlVal, FunctionSignature, IntoStahlVal, MutFunctionSignature, Result,
        StahlVal, TypeKind,
    },
...
/// A module to be consumed by the Stahl Engine for later on demand access by scripts
...
struct BuiltInModuleRepr {
    ...
    values: std::collections::HashMap<Arc<str>, StahlVal, FxBuildHasher>,
...
pub static VOID_MODULE: Lazy<InternedString> =
    Lazy::new(|| "%-builtin-module-stahl/constants".into());
...
impl BuiltInModuleRepr {
    ...
    pub fn search(&self, value: StahlVal) -> Option<FunctionSignatureMetadata> {
        match value {
            StahlVal::FuncV(f) => self
                .fn_ptr_table
                .get(&BuiltInFunctionType::Reference(f))
                .cloned(),
            StahlVal::MutFunc(f) => self
                .fn_ptr_table
    ...
    pub fn bound_identifiers(&self) -> crate::values::lists::List<StahlVal> {
        self.values
            .keys()
            .map(|x| StahlVal::StringV(x.to_string().into()))
            .collect()
    }
    ...
    /// Add a value to the module namespace. This value can be any legal StahlVal, or if you're explicitly attempting
    /// to compile an program for later use and don't currently have access to the functions in memory, use `StahlVal::Void`
    ...
    pub fn get(&self, name: String) -> StahlVal {
    ...
    pub fn try_get(&self, name: String) -> Option<StahlVal> {
...
impl BuiltInModule {
    ...
    pub fn register_native_fn_definition(
        ...
        let stahl_val = match definition.func {
            BuiltInFunctionType::Reference(value) => StahlVal::FuncV(value),
            BuiltInFunctionType::Mutable(value) => StahlVal::MutFunc(value),
            BuiltInFunctionType::Context(value) => StahlVal::BuiltIn(value),
        };

        let names = std::iter::once(definition.name).chain(definition.aliases.into_iter().cloned());

        for name in names {
            self.register_value(name, stahl_val.clone());
        }
