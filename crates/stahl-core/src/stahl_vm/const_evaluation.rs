use crate::compiler::program::number_literal_to_stahl;
...
struct ConstantEnv {
    bindings: HashMap<InternedString, StahlVal, FxBuildHasher>,
...
impl ConstantEnv {
    fn root(bindings: HashMap<InternedString, StahlVal, FxBuildHasher>) -> Self {
        Self {
            bindings,
    ...
    fn bind(&mut self, ident: &InternedString, value: StahlVal) {
    ...
    fn get(&mut self, ident: &InternedString) -> Option<StahlVal> {
    ...
    fn _set(&mut self, ident: &InternedString, value: StahlVal) -> Option<StahlVal> {
...
impl<'a> ConstantEvaluatorManager<'a> {
    pub fn new(
        ...
        constant_bindings: HashMap<InternedString, StahlVal, FxBuildHasher>,
        opt_level: OptLevel,
        kernel: &'a mut Option<Kernel>,
    ) -> Self {
        Self {
...
fn stahlval_to_atom(value: &StahlVal) -> Option<TokenType<InternedString>> {
    match value {
        StahlVal::BoolV(b) => Some(TokenType::BooleanLiteral(*b)),
        StahlVal::NumV(n) => Some(RealLiteral::Float(*n).into()),
        StahlVal::CharV(c) => Some(TokenType::CharacterLiteral(*c)),
        StahlVal::IntV(i) => Some(IntLiteral::Small(*i).into()),
        StahlVal::StringV(s) => Some(TokenType::StringLiteral(s.to_arc_string())),
        _ => None,
    }
}
...
impl<'a> ConstantEvaluator<'a> {
    ...
    fn to_constant(&self, expr: &ExprKind) -> Option<StahlVal> {
        match expr {
            ExprKind::Atom(Atom { syn, .. }) => self.eval_atom(syn),
            ExprKind::Quote(q) => {
                let inner = &q.expr;
                TryFromExprKindForStahlVal::try_from_expr_kind(inner.clone()).ok()
            }
            _ => None,
        }
    }

    fn eval_atom(&self, t: &SyntaxObject) -> Option<StahlVal> {
        match &t.ty {
            ...
            TokenType::Number(n) => number_literal_to_stahl(n).ok(),
            TokenType::StringLiteral(s) => Some(StahlVal::StringV((s.clone()).into())),
            TokenType::CharacterLiteral(c) => Some(StahlVal::CharV(*c)),
            _ => None,
        }
    }

    fn all_to_constant(&self, exprs: &[ExprKind]) -> Option<smallvec::SmallVec<[StahlVal; 8]>> {
    ...
    fn eval_kernel_function(
        ...
        if let Some(new_token) = stahlval_to_atom(&output) {
        ...
        if evaluated_func.is_function() {
            match evaluated_func {
                ...
                StahlVal::FuncV(f) => {
                    ...
                    // .memoization_table
                    // .get(StahlVal::FuncV(f), args.to_vec())
                    // {
                    // output
                    // } else {
    ...
    fn handle_output(
        ...
        // evaluated_func: &StahlVal,
        mut raw_args: Vec<ExprKind>,
    ) -> std::result::Result<ExprKind, crate::StahlErr> {
        if let Some(new_token) = stahlval_to_atom(&output) {
            let atom = Atom::new(SyntaxObject::new(new_token, get_span(&func)));
            // debug!(
            //     "Const evaluation of a function resulted in an atom: {}",
...
impl<'a> ConsumingVisitor for ConstantEvaluator<'a> {
    ...
    fn visit_atom(&mut self, a: crate::parser::ast::Atom) -> Self::Output {
        ...
        //     if let Some(new_token) = stahlval_to_atom(&inner) {
            ...
            if l.rest {
                if let Some((l_last, l_start)) = l.args.split_last() {
                    ...
                    if rest_args.len() == args[l_start.len()..].len() {
                        let list = StahlVal::ListV(rest_args.into());
