...
use crate::{
    ...
    rvals::{IntoStahlVal, StahlString, StahlVal},
};
...
pub fn load_module(target: &StahlString) -> crate::rvals::Result<StahlVal> {
    match DylibContainers::load_module(target.clone()) {
        Some(container) => container.into_stahlval(),
        None => {
            stop!(Generic => format!("dylib not found: {} or dylibs are not enabled for this instance of the stahl runtime", target))
        }
    }
}
...
impl DylibContainers {
    ...
    // home should... probably just be $STAHL_HOME?
    pub fn load_module(target: StahlString) -> Option<BuiltInModule> {
            ...
            if let Some(home) = home {
                ...
                if home.exists() {
                    ...
                    for path in paths {
                        ...
                        let external_module =
                            crate::stahl_vm::ffi::FFIWrappedModule::new(dylib_module)
                    ...
                    log::warn!(target: "dylibs", "$STAHL_HOME/native directory does not exist")
                }
            } else {
                log::warn!(target: "dylibs", "STAHL_HOME variable missing - unable to read shared dylibs")
            }
