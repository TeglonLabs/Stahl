...
use super::{
    ...
    vm::{StahlThread, Synchronizer, ThreadStateController},
};
...
use crate::{
    compiler::{
        ...
        program::{
            number_literal_to_stahl, Executable, RawProgramWithSymbols,
    ...
    rvals::{
        AsRefMutStahlVal, AsRefStahlVal as _, FromStahlVal, IntoStahlVal, MaybeSendSyncStatic,
        Result, StahlString, StahlVal,
    },
    stahl_vm::register_fn::RegisterFn,
    stop, throw,
    values::{
...
/// Handle to a stahl engine. This contains a main entrypoint thread, alongside
/// the compiler and all of the state necessary to keep a VM instance alive and
/// well.
pub struct Engine {
    pub(crate) virtual_machine: StahlThread,
    // TODO: Just put this, and all the other things,
    // inside the `StahlThread` - The compiler probably
...
impl<'a> LifetimeGuard<'a> {
    ...
    pub fn consume<T>(self, mut thunk: impl FnMut(&mut Engine, Vec<StahlVal>) -> T) -> T {
        let values =
            crate::gc::unsafe_erased_pointers::OpaqueReferenceNursery::drain_weak_references_to_stahlvals();

        thunk(self.engine, values)
    }

    pub fn consume_once<T>(self, mut thunk: impl FnOnce(&mut Engine, Vec<StahlVal>) -> T) -> T {
        let values =
            crate::gc::unsafe_erased_pointers::OpaqueReferenceNursery::drain_weak_references_to_stahlvals();
...
#[stahl_derive::function(name = "#%get-dylib")]
pub fn load_module_noop(target: &crate::rvals::StahlString) -> crate::rvals::Result<StahlVal> {
...
static STATIC_DEFAULT_PRELUDE_MACROS: OnceCell<FxHashMap<InternedString, StahlMacro>> =
    OnceCell::new();

static STATIC_DEFAULT_PRELUDE_MACROS_SANDBOX: OnceCell<FxHashMap<InternedString, StahlMacro>> =
    OnceCell::new();

pub(crate) fn set_default_prelude_macros(
    prelude_macros: FxHashMap<InternedString, StahlMacro>,
    sandbox: bool,
) {
    if cfg!(feature = "sync") {
        if sandbox {
...
pub(crate) fn default_prelude_macros() -> FxHashMap<InternedString, StahlMacro> {
    if cfg!(feature = "sync") {
...
thread_local! {
    ...
    pub(crate) static DEFAULT_PRELUDE_MACROS: RefCell<FxHashMap<InternedString, StahlMacro>> = RefCell::new(HashMap::default());
}
...
impl Engine {
    ...
    pub(crate) fn new_kernel(sandbox: bool) -> Self {
        ...
        let mut vm = Engine {
            virtual_machine: StahlThread::new(sources.clone(), compiler),
            modules,
            sources,
            #[cfg(feature = "dylibs")]
        ...
        time!(
            ...
            vm.compile_and_run_raw_program(crate::stahl_vm::primitives::ALL_MODULES)
    ...
    pub fn disallow_dylib_loading(&mut self) -> &mut Self {
        ...
        if let Some(builtin_module) = module.get_mut("stahl/meta") {
    ...
    pub(crate) fn new_bootstrap_kernel(sandbox: bool) -> Self {
        ...
        if matches!(option_env!("STAHL_BOOTSTRAP"), Some("false") | None) {
            let mut vm = Engine::new_kernel(sandbox);

            let sources = vm.sources.clone();

            vm.register_fn("report-error!", move |error: StahlErr| {
    ...
    pub fn execute_non_interactive_program_image(program: &'static [u8]) -> Result<()> {
        ...
        let mut engine = Engine::new();
        let program = crate::stahl_vm::engine::NonInteractiveProgramImage::from_bytes(program);
    ...
    //     let mut vm = Engine {
    //         virtual_machine: StahlThread::new(sources.clone()),
    ...
    //         crate::stahl_vm::primitives::ALL_MODULES,
    ...
    //     let builtin_modules =
    //         ["(require \"#%private/stahl/contract\" (for-syntax \"#%private/stahl/contract\"))"];
    ...
    /// Registers a stahl module
    pub fn register_stahl_module(&mut self, module_name: String, text: String) {
    ...
    pub fn call_printing_method_in_context(&mut self, argument: StahlVal) -> Result<StahlVal> {
        ...
        T: ReferenceMarker<'b, Static = EXT>,
    {
        self.with_mut_reference(obj).consume(move |engine, args| {
            ...
            engine.update_value(bind_to, StahlVal::Void);

            res.map(|x| x.into_iter().next().unwrap_or(StahlVal::Void))
        })
    }
    ...
    >(
        &'a mut self,
        obj: &'a mut T,
        mut thunk: impl FnMut(&mut Engine, StahlVal) -> Result<StahlVal>,
    ) -> Result<StahlVal>
    where
        T: ReferenceMarker<'b, Static = EXT>,
    {
        self.with_mut_reference(obj).consume(|engine, args| {
            ...
            thunk(engine, args.into_iter().next().unwrap_or(StahlVal::Void))
        })
    }
    ...
    ///
    /// By default, the engine will search $STAHL_HOME/cogs for modules,
    ...
    pub fn register_external_module(
        ...
        self.register_value(
            ...
            external_module.into_stahlval().unwrap(),
        );
    ...
    fn load_raw_program(&mut self, mut program: RawProgramWithSymbols) {
        fn eval_atom(t: &SyntaxObject) -> Result<StahlVal> {
            match &t.ty {
                TokenType::BooleanLiteral(b) => Ok((*b).into()),
                TokenType::Number(n) => number_literal_to_stahl(n),
                TokenType::StringLiteral(s) => Ok(StahlVal::StringV(s.clone().into())),
                TokenType::CharacterLiteral(c) => Ok(StahlVal::CharV(*c)),
                // TODO: Keywords shouldn't be misused as an expression - only in function calls are keywords allowed
                TokenType::Keyword(k) => Ok(StahlVal::SymbolV(k.clone().into())),
                what => {
        ...
        for expr in &mut program.instructions {
            ...
            for instruction in expr.iter_mut() {
                ...
                //         ..
                //     } => {
                //         let value = StahlVal::try_from(expression.clone())
    ...
    pub fn run_raw_program(&mut self, program: RawProgramWithSymbols) -> Result<Vec<StahlVal>> {
    ...
    pub fn run_executable(&mut self, executable: &Executable) -> Result<Vec<StahlVal>> {
        self.virtual_machine.run_executable(executable)
    }
    ...
    /// Registers an external value of any type as long as it implements [`FromStahlVal`](crate::rvals::FromStahlVal) and
    /// [`IntoStahlVal`](crate::rvals::IntoStahlVal). This method does the coercion to embed the type into the `Engine`'s
    /// environment with the name `name`. This function can fail only if the conversion from `T` to [`StahlVal`](crate::rvals::StahlVal) fails.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    pub fn register_external_value<T: FromStahlVal + IntoStahlVal>(
        &mut self,
        name: &str,
        value: T,
    ) -> Result<&mut Self> {
        let converted = value.into_stahlval()?;
        Ok(self.register_value(name, converted))
    }

    /// Registers a [`StahlVal`](crate::rvals::StahlVal) under the name `name` in the `Engine`'s internal environment.
    ///
    /// # Examples
    /// ```
    /// # extern crate stahl;
    /// # use stahl::stahl_vm::engine::Engine;
    /// use stahl::rvals::StahlVal;
    ///
    /// let mut vm = Engine::new();
    /// let external_value = StahlVal::StringV("hello-world".to_string().into());
    ...
    pub fn update_value(&mut self, name: &str, value: StahlVal) -> Option<&mut Self> {
    ...
    pub fn register_values(
        &mut self,
        values: impl Iterator<Item = (String, StahlVal)>,
    ) -> &mut Self {
    ...
    /// In order to be registered, a type must implement [`FromStahlVal`](crate::rvals::FromStahlVal)
    /// and [`IntoStahlVal`](crate::rvals::IntoStahlVal)
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    pub fn register_type<T: FromStahlVal + IntoStahlVal>(
        &mut self,
        ...
        let f = move |args: &[StahlVal]| -> Result<StahlVal> {
            ...
            Ok(StahlVal::BoolV(T::from_stahlval(&args[0]).is_ok()))
        };
    ...
    /// This will return the [`StahlVal`](crate::rvals::StahlVal), not the underlying data.
    /// To unwrap the value, use the [`extract`](crate::stahl_vm::engine::Engine::extract) method and pass the type parameter.
    ///
    ...
    /// assert_eq!(vm.extract_value("a").unwrap(), StahlVal::IntV(10));
    /// ```
    pub fn extract_value(&self, name: &str) -> Result<StahlVal> {
    ...
    /// if the type passed in does not match the value (and thus the coercion using [`FromStahlVal`](crate::rvals::FromStahlVal) fails)
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ```
    pub fn extract<T: FromStahlVal>(&self, name: &str) -> Result<T> {
        T::from_stahlval(&self.extract_value(name)?)
    }

    /// Raise the error within the stack trace
    pub fn raise_error(&self, error: StahlErr) {
        raise_error(&self.sources, error)
    }

    /// Emit an error string reporing, the back trace.
    pub fn raise_error_to_string(&self, error: StahlErr) -> Option<String> {
        raise_error_to_string(&self.sources, error)
    }

    /// Execute a program given as the `expr`, and computes a `Vec<StahlVal>` corresponding to the output of each expression given.
    ...
    /// assert_eq!(output, vec![StahlVal::IntV(3), StahlVal::IntV(25), StahlVal::IntV(5)]);
    /// ```
    // pub fn run(&mut self, expr: &str) -> Result<Vec<StahlVal>> {
    ...
    // pub fn run_without_callbacks(&mut self, expr: &str) -> Result<Vec<StahlVal>> {
    //     let constants = self.constants();
    //     let program = self.compiler.compile_program(expr, None, constants)?;
    //     self.virtual_machine
...
fn raise_error(sources: &Sources, error: StahlErr) {
...
pub(crate) fn raise_error_to_string(sources: &Sources, error: StahlErr) -> Option<String> {
    if let Some(span) = error.span() {
        let source_id = span.source_id();
        if let Some(source_id) = source_id {
            let sources = sources.sources.lock().unwrap();
...
mod engine_api_tests {
    ...
    fn test_references_in_engine() {
        ...
        engine.register_value("*external*", StahlVal::Void);
...

        {
            ...
            assert_eq!(res, StahlVal::IntV(10));
        }
    }
    ...
    fn test_references_in_engine_get_removed_after_lifetime() {
        ...
        assert_eq!(engine.extract_value("*external*").unwrap(), StahlVal::Void);
    }
...
mod derive_macro_tests {
    ...
    #[derive(stahl_derive::_Stahl, PartialEq, Debug)]
    #[stahl(equality, getters, constructors)]
