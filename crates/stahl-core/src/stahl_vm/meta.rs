...
// pub type BuiltInSignature = fn(Vec<StahlVal>, &mut dyn VmContext) -> Result<StahlVal>;`

use std::borrow::Cow;
use std::{cell::RefCell, convert::TryFrom, io::Write, rc::Rc};

use crate::gc::shared::ShareableMut;
use crate::parser::tryfrom_visitor::TryFromExprKindForStahlVal;
// use im_lists::list::List;
use crate::values::lists::List;

use crate::values::port::StahlPortRepr;
use crate::values::structs::StahlResult;
use crate::{
    parser::ast::ExprKind,
    rvals::Custom,
    values::port::{StahlPort, CAPTURED_OUTPUT_PORT, DEFAULT_OUTPUT_PORT},
    StahlErr, StahlVal,
};
use crate::{parser::expander::LocalMacroManager, rvals::Result};
use crate::{parser::parser::ParseError, stahl_vm::engine::Engine};
...
impl EngineWrapper {
    ...
    pub(crate) fn raise_error(&self, error: StahlErr) {
    ...
    pub(crate) fn call_fn(&mut self, function_name: StahlVal, args: StahlVal) -> Result<StahlVal> {
        let function = match function_name {
            StahlVal::SymbolV(expr) | StahlVal::StringV(expr) => {
        ...
        if let StahlVal::ListV(list) = args {
    ...
    fn call_impl(&mut self, expr: StahlVal) -> Result<List<StahlVal>> {
        match expr {
            StahlVal::StringV(expr) => self
                .0
            ...
            StahlVal::ListV(list) => {
                let values = list
                    ...
                    .collect::<Result<Vec<Vec<StahlVal>>>>();
    ...
    pub(crate) fn call(&mut self, expr: StahlVal) -> StahlResult<List<StahlVal>, StahlErr> {
        self.call_impl(expr).into()
    }

    // TODO: Warning, here be dragons
    pub(crate) fn get_value(&self, expr: StahlVal) -> Result<StahlVal> {
        match expr {
            StahlVal::SymbolV(expr) | StahlVal::StringV(expr) => {
                self.0.extract_value(expr.as_ref())
            }
...
pub fn read(args: &[StahlVal]) -> Result<StahlVal> {
    if args.len() != 1 {
    ...
    args[0]
        ...
        .into_iter()
        .map(TryFromExprKindForStahlVal::try_from_expr_kind)
...
pub fn expand_macros(arguments: &[StahlVal]) -> Result<StahlVal> {
    if arguments.len() != 1 {
...
pub fn value_to_string(value: StahlVal) -> String {
    format!("{value:?}")
}
...
pub fn eval(program: String) -> List<StahlVal> {
    ...
    match res {
        Ok(v) => vec![
            StahlVal::ListV(v.into()),
            StahlVal::StringV(drain_custom_output_port().into()),
            StahlVal::StringV("".into()),
        ]
        .into(),
        Err(e) => {
            ...
            vec![
                StahlVal::ListV(List::new()),
                StahlVal::StringV(drain_custom_output_port().into()),
                StahlVal::StringV(report.into()),
            ]
            .into()
        }
    }
