...
pub(crate) const TRANSDUCE: StahlVal = StahlVal::BuiltIn(transduce);

// figure out if nested transducers works
fn transduce(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    if args.len() < 2 {
    ...
    if let StahlVal::ReducerV(r) = &reducer {
...
impl<'global, 'a> VmCore<'a> {
    ...
    pub(crate) fn res_iterator(
        ...
        nursery: &'global mut Option<Vec<StahlVal>>,
    ) -> Result<Box<dyn Iterator<Item = Result<StahlVal>> + 'global>> {
        match value {
            StahlVal::VectorV(v) => Ok(Box::new(v.iter().cloned().map(Ok))),
            StahlVal::StreamV(lazy_stream) => Ok(Box::new(LazyStreamIter::new(
                lazy_stream.unwrap(),
                vm_ctx,
                cur_inst_span,
            ))),
            StahlVal::StringV(s) => Ok(Box::new(s.chars().map(|x| Ok(StahlVal::CharV(x))))),
            StahlVal::ListV(l) => Ok(Box::new(l.clone().into_iter().map(Ok))),
            StahlVal::HashSetV(hs) => Ok(Box::new(hs.iter().cloned().map(Ok))),
            StahlVal::HashMapV(hm) => {
                Ok(Box::new(hm.iter().map(|x| {
                    Ok(StahlVal::ListV(vec![x.0.clone(), x.1.clone()].into()))
                })))
            }
        ...
        for t in ops {
            iter = match t {
                Transducers::Map(stack_func) => {
                    ...
                    match stack_func {
                        StahlVal::FuncV(func) => Box::new(iter.map(move |arg| {
                            let arg_vec = [arg?];
                            func(&arg_vec).map_err(|x| x.set_span_if_none(*cur_inst_span))
                        })),
                        StahlVal::BoxedFunction(func) => Box::new(iter.map(move |arg| {
                            let arg_vec = [arg?];
                            func.func()(&arg_vec).map_err(|x| x.set_span_if_none(*cur_inst_span))
                        })),
                        StahlVal::MutFunc(func) => Box::new(iter.map(move |arg| {
                            let mut arg_vec = [arg?];
                ...
                Transducers::Filter(stack_func) => {
                    ...
                    let switch_statement = move |arg: Result<StahlVal>| match arg {
                        Ok(arg) => {
                ...
                Transducers::FlatMap(stack_func) => {
                    ...
                    let switch_statement =
                        move |arg: Result<StahlVal>| -> Box<dyn Iterator<Item = Result<StahlVal>>> {
                            match arg {
                                Ok(arg) => {
                                    let res = vm_copy.borrow_mut().call_func_or_else(
                                    stack_func,
                                    ...
                                    match res {
                                        Ok(x) => {
                                            match x {
                                                ...
                                                StahlVal::StringV(s) => Box::new(
                                                    s.chars()
                                                        .map(|x| Ok(StahlVal::CharV(x)))
                                                        .collect::<Vec<_>>()
                                                        .into_iter(),
                                                ),
                                                StahlVal::ListV(l) => {
                                                ...
                                                // }
                                                els => {
                                                    let err = StahlErr::new(ErrorKind::TypeMismatch, format!("flatten expected a traversable value, found: {els}")).with_span(*cur_inst_span);
                            ...
                            match arg {
                                Ok(x) => {
                                    match x {
                                        ...
                                        StahlVal::ListV(l) => Box::new(l.into_iter().map(Ok)),
                                        // StahlVal::StructV(s) => {
                                        //     Box::new(s.unwrap().fields.into_iter().map(Ok))
                                        // }
                                        els => {
                ...
                Transducers::Extend(collection) => {
                    let extender: Box<dyn Iterator<Item = Result<StahlVal>>> =
                        match collection.clone() {
                            StahlVal::VectorV(v) => Box::new(v.0.unwrap().into_iter().map(Ok)),
                            // TODO this needs to be fixed
                            StahlVal::StringV(s) => Box::new(
                                s.chars()
                                    .map(|x| Ok(StahlVal::CharV(x)))
                            ...
                            StahlVal::ListV(l) => Box::new(l.into_iter().map(Ok)),
                            // StahlVal::StructV(s) => Box::new(s.unwrap().fields.into_iter().map(Ok)),
                            els => {
                                let err = StahlErr::new(
                ...
                Transducers::Enumerating => Box::new(iter.enumerate().map(|x| {
                    Ok(StahlVal::ListV(
                        vec![StahlVal::IntV(x.0 as isize), x.1?].into(),
                    ))
                })),
                Transducers::Zipping(collection) => {
                    let zipped: Box<dyn Iterator<Item = Result<StahlVal>>> =
                        match collection.clone() {
                            StahlVal::VectorV(v) => Box::new(v.0.unwrap().into_iter().map(Ok)),
                            // TODO this needs to be fixed
                            StahlVal::StringV(s) => Box::new(
                    ...
                    Box::new(
                        iter.zip(zipped)
                            .map(|x| Ok(StahlVal::ListV(vec![x.0?, x.1?].into()))),
                    )
                }
                Transducers::Interleaving(collection) => {
                    let other: Box<dyn Iterator<Item = Result<StahlVal>>> = match collection.clone()
                    {
                        StahlVal::VectorV(v) => Box::new(v.0.unwrap().into_iter().map(Ok)),
                        // TODO this needs to be fixed
                        StahlVal::StringV(s) => Box::new(
    ...
    fn into_value(
        vm_ctx: Rc<RefCell<&'global mut Self>>,
        reducer: Reducer,
        mut iter: impl Iterator<Item = Result<StahlVal>>,
        cur_inst_span: &Span,
    ) -> Result<StahlVal> {
        match reducer {
            ...
            Reducer::Min => todo!(),
            Reducer::Count => {
                Ok(StahlVal::IntV(iter.count().try_into().unwrap())) // TODO have proper big int
            },
            Reducer::Nth(usize) => {
                iter.nth(usize).unwrap_or_else(|| stop!(Generic => "`nth` - index given is greater than the length of the iterator"))
            },
            Reducer::List => iter.collect::<Result<List<_>>>().map(StahlVal::ListV),
            ...
            Reducer::HashMap => {
                ...
                }).collect::<Result<HashMap<_, _>>>().map(|x| StahlVal::HashMapV(Gc::new(x).into()))
            },
            Reducer::HashSet => iter.collect::<Result<HashSet<_>>>().map(|x| StahlVal::HashSetV(Gc::new(x).into())),
            Reducer::String => todo!(),
