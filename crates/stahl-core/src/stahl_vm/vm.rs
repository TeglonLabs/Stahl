...
use crate::primitives::lists::stahl_cons;
use crate::primitives::numbers::add_two;
use crate::rvals::as_underlying_type;
use crate::rvals::cycles::BreadthFirstSearchStahlValVisitor;
use crate::rvals::number_equality;
use crate::rvals::BoxedAsyncFunctionSignature;
use crate::rvals::FromStahlVal as _;
use crate::rvals::StahlString;
use crate::stahl_vm::primitives::stahl_not;
use crate::stahl_vm::primitives::stahl_set_box_mutable;
use crate::stahl_vm::primitives::stahl_unbox_mutable;
...
use crate::{
    ...
    stahl_vm::primitives::{equality_primitive, lte_primitive},
...
use crate::rvals::{from_serializable_value, into_serializable_value, IntoStahlVal};
...
pub struct StackFrameAttachments {
    pub(crate) handler: Option<StahlVal>,
...
fn check_sizes() {
    ...
    println!(
        "option rc stahlval: {:?}",
        std::mem::size_of::<Option<Rc<StahlVal>>>()
    );
    println!(
        "option box stahlval: {:?}",
        std::mem::size_of::<Option<Box<StahlVal>>>()
    );
    println!(
        "option stahlval: {:?}",
        std::mem::size_of::<Option<StahlVal>>()
    );
}
...
impl StackFrame {
    ...
    pub fn with_handler(mut self, handler: StahlVal) -> Self {
...
pub struct StahlThread {
    ...
    pub(crate) thread_local_storage: Vec<StahlVal>,
...
struct ThreadContext {
    ctx: std::sync::Weak<AtomicCell<Option<*mut StahlThread>>>,
    handle: StahlVal,
}
...
pub struct Synchronizer {
    ...
    ctx: Arc<AtomicCell<Option<*mut StahlThread>>>,
}
...
impl Synchronizer {
    ...
    pub(crate) unsafe fn call_per_ctx(&mut self, mut func: impl FnMut(&mut StahlThread)) {
    ...
    pub fn stop_threads(&mut self) {
        ...
        self.threads.lock().unwrap().iter().for_each(|x| {
            if let StahlVal::Custom(c) = &x.handle {
...
impl StahlThread {
    pub fn new(sources: Sources, compiler: std::sync::Arc<RwLock<Compiler>>) -> StahlThread {
    ...
    pub fn enter_safepoint(
        &mut self,
        mut finish: impl FnMut(&StahlThread) -> Result<StahlVal>,
    ) -> Result<StahlVal> {
        // TODO:
    ...
    pub fn insert_binding(&mut self, idx: usize, value: StahlVal) {
        self.global_env.add_root_value(idx, value);
    }

    pub fn extract_value(&self, idx: usize) -> Option<StahlVal> {
        self.global_env.extract(idx)
    }

    // Run the executable
    pub fn run_executable(&mut self, program: &Executable) -> Result<Vec<StahlVal>> {
        let Executable {
            instructions,
            constant_map,
            spans,
    ...
    pub(crate) fn call_function_from_mut_slice(
        ...
        match function {
            StahlVal::FuncV(func) => func(args).map_err(|x| x.set_span_if_none(Span::default())),
            StahlVal::BoxedFunction(func) => {
                func.func()(args).map_err(|x| x.set_span_if_none(Span::default()))
            }
            // StahlVal::ContractedFunction(cf) => {
            //     let arg_vec: Vec<_> = args.into_iter().collect();
            //     cf.apply(arg_vec, cur_inst_span, self)
            // }
            StahlVal::MutFunc(func) => func(args).map_err(|x| x.set_span_if_none(Span::default())),
            // StahlVal::BuiltIn(func) => {
            //     let arg_vec: Vec<_> = args.into_iter().collect();
            //     func(self, &arg_vec).map_err(|x| x.set_span_if_none(*cur_inst_span))
            // }
            StahlVal::CustomStruct(ref s) => {
                if let Some(procedure) = s.maybe_proc() {
                    if let StahlVal::HeapAllocated(h) = procedure {
    ...
    pub(crate) fn call_function(
        ...
        match function {
            ...
            StahlVal::ContinuationFunction(c) => {
                let mut vm_instance = VmCore::new_unchecked(
                    RootedInstructions::new(THE_EMPTY_INSTRUCTION_SET.with(|x| x.clone())),
                    constant_map,
                    self,
            ...
            if let Err(e) = result {
                while let Some(mut last) = vm_instance.thread.stack_frames.pop() {
                    ...
                    if let Some(handler) = last.attachments.as_mut().and_then(|x| x.handler.take())
                    {
                        ...
                        vm_instance.thread.stack.push(e.into_stahlval()?);
...
pub struct OpenContinuationMark {
    ...
    pub(crate) current_stack_values: Vec<StahlVal>,
...
// pub type BuiltInSignature2 = fn(Vec<StahlVal>, &mut dyn VmContext) -> Result<StahlVal>;

// These reference the current existing thread
// TODO: Change this to refer directly to StahlThread in some way
pub type BuiltInSignature =
    for<'a, 'b> fn(&'a mut VmCore<'b>, &[StahlVal]) -> Option<Result<StahlVal>>;
...
pub struct VmCore<'a> {
    ...
    pub(crate) thread: &'a mut StahlThread,
    pub(crate) root_spans: &'a [Span],
}

// TODO: Delete this entirely, and just have the run function live on top of the StahlThread.
//
impl<'a> VmCore<'a> {
    fn new_unchecked(
        instructions: RootedInstructions,
        constants: ConstantMap,
        thread: &'a mut StahlThread,
        root_spans: &'a [Span],
    ) -> VmCore<'a> {
        VmCore {
            instructions,
    ...
    #[cfg(feature = "sync")]
    pub fn stahl_function_to_rust_function(
        &self,
        func: StahlVal,
    ) -> Box<dyn Fn(&mut [StahlVal]) -> Result<StahlVal> + Send + Sync + 'static> {
        let thread = Arc::new(Mutex::new(self.make_thread()));
        let rooted = func.as_rooted();

        Box::new(move |args: &mut [StahlVal]| {
    ...
    #[cfg(feature = "sync")]
    pub fn make_thread(&self) -> StahlThread {
    ...
    pub fn make_box(&mut self, value: StahlVal) -> StahlVal {
        ...
        StahlVal::HeapAllocated(allocated_var)
    }

    pub fn make_mutable_vector(&mut self, values: Vec<StahlVal>) -> StahlVal {
        ...
        StahlVal::MutableVector(allocated_var)
    }
            ...
            if let Err(e) = result {
                while let Some(mut last) = self.thread.stack_frames.pop() {
                    ...
                    if let Some(handler) = last.attachments.as_mut().and_then(|x| x.handler.take())
                    {
                        ...
                        self.thread.stack.push(e.into_stahlval()?);
    ...
    // #[inline(always)]
    pub(crate) fn call_func_or_else<F: FnOnce() -> StahlErr>(
        &mut self,
        func: &StahlVal,
        arg: StahlVal,
        cur_inst_span: &Span,
        ...
        match func {
            ...
            StahlVal::Closure(closure) => self.call_with_one_arg(closure, arg),
            _ => Err(err()),
        }
    }

    // #[inline(always)]
    pub(crate) fn call_func_or_else_two_args<F: FnOnce() -> StahlErr>(
        &mut self,
        func: &StahlVal,
        arg1: StahlVal,
        arg2: StahlVal,
        ...
        match func {
            ...
            //     func(self, &arg_vec).map_err(|x| x.set_span_if_none(*cur_inst_span))
            // }
            StahlVal::Closure(closure) => self.call_with_two_args(closure, arg1, arg2),
            _ => Err(err()),
        }
    }

    // #[inline(always)]
    pub(crate) fn call_func_or_else_many_args<F: FnOnce() -> StahlErr>(
        &mut self,
        func: &StahlVal,
        args: impl IntoIterator<Item = StahlVal>,
        cur_inst_span: &Span,
        err: F,
    ) -> Result<StahlVal> {
        match func {
            ...
            //     func(self, &arg_vec).map_err(|x| x.set_span_if_none(*cur_inst_span))
            // }
            StahlVal::Closure(closure) => self.call_with_args(closure, args),
            _ => Err(err()),
        }
    }
    ...
    // pub(crate) fn eval_executable(&mut self, executable: &Executable) -> Result<Vec<StahlVal>> {
    ...
    pub(crate) fn vm(&mut self) -> Result<StahlVal> {
        // if self.depth > 1024 {
        ...
        macro_rules! inline_register_primitive_immediate {
            ($name:tt) => {{
                ...
                let const_val = StahlVal::IntV(push_const.payload_size.to_usize() as isize);
        ...
        loop {
            ...
            match instr {
                ...
                DenseInstruction {
                    ...
                    let result = match subtract_primitive(&[local_value, StahlVal::IntV(1)]) {
                        Ok(value) => value,
                        Err(e) => return Err(e.set_span_if_none(self.current_span())),
                    };
                    ...
                    let result = match l {
                        ...
                        StahlVal::IntV(l) => {
                            match l.checked_sub(&r) {
                                Some(r) => StahlVal::IntV(r),
                                // Slow path
                                None => StahlVal::BigNum(Gc::new(BigInt::from(*l) - r)),
                            }
                        }
                        ...
                        | StahlVal::BigRational(_) => {
                            ...
                            subtract_primitive(&[l.clone(), StahlVal::IntV(r)])
                ...
                DenseInstruction {
                    ...
                    let result = match l {
                        ...
                        | StahlVal::Rational(_)
                        | StahlVal::BigNum(_)
                        | StahlVal::BigRational(_) => l.clone() <= StahlVal::IntV(r),
                        _ => {
                            stop!(TypeMismatch => format!("lte expected an number, found: {}", l); self.current_span())
                        }
                    };

                    self.thread.stack.push(StahlVal::BoolV(result));
                ...
                DenseInstruction {
                    ...
                    self.thread.stack.push(StahlVal::Void);
                    self.ip += 1;
                }
                DenseInstruction {
                    op_code: OpCode::SET,
                ...
                DenseInstruction {
                    ...
                    self.thread.stack.push(StahlVal::INT_ZERO);
                    self.ip += 1;
                }
                DenseInstruction {
                    ...
                    self.thread.stack.push(StahlVal::INT_ONE);
                    self.ip += 1;
                }
                DenseInstruction {
                    ...
                    self.thread.stack.push(StahlVal::INT_TWO);
                    self.ip += 1;
                }
                ...
                DenseInstruction {
                    ...
                    // self.thread.stack.push(StahlVal::INT_TWO);
                    self.ip += 1;
                    if let Some(r) = self.handle_pop_pure_value(StahlVal::INT_ONE) {
                        return r;
                    }
                }
                ...
                DenseInstruction {
                    ...
                    match stack_func {
                        FuncV(f) => {
                            ...
                            let result =
                                match self.thread.enter_safepoint(move |ctx: &StahlThread| {
                ...
                DenseInstruction {
                    ...
                    if likely(!is_multi_arity) {
                        ...
                        let list = StahlVal::ListV(values);
                ...
                DenseInstruction {
                    ...
                    let val = if bytes {
                        ...
                        StahlVal::ByteVector(crate::rvals::StahlByteVector::new(buffer))
                    } else {
                        StahlVal::VectorV(crate::rvals::StahlVector(Gc::new(args.into())))
                    };
    ...
    fn move_from_stack(&mut self, offset: usize) -> StahlVal {
        std::mem::replace(&mut self.thread.stack[offset], StahlVal::Void)
    }
    ...
    #[inline(always)]
    fn handle_pop_pure_value(&mut self, value: StahlVal) -> Option<Result<StahlVal>> {
        ...
        if should_continue {
            ...
            //     StahlErr::new(ErrorKind::Generic, "stack empty at pop".to_string())
    ...
    #[inline(always)]
    fn handle_pop_pure(&mut self) -> Option<Result<StahlVal>> {
        // Check that the amount we're looking to pop and the function stack length are equivalent
        // otherwise we have a problem
        // println!("{} - {}", self.pop_count, self.thread.stack_frames.len());
        self.pop_count -= 1;
    ...
    fn handle_pure_function(&mut self, offset: usize) {
            ...
            .get(&closure_id)
        {
                ...
                .and_then(|x| self.thread.function_interner.spans.get(&x.function.id))
            {
                ...
                λ > (the-next-cont 10)
                hello world
                thread 'main' panicked at crates/stahl-core/src/stahl_vm/vm.rs:2699:32:
        ...
        self.thread
            .stack
            .push(StahlVal::Closure(constructed_lambda));
    ...
    fn handle_new_start_closure(&mut self, offset: usize) -> Result<()> {
        ...
        self.thread
            .stack
            .push(StahlVal::Closure(Gc::new(constructed_lambda)));
    ...
    #[inline(always)]
    fn handle_tail_call(&mut self, stack_func: StahlVal, payload_size: usize) -> Result<()> {
        use StahlVal::*;
    ...
    fn call_boxed_func(
        &mut self,
        func: &dyn Fn(&[StahlVal]) -> Result<StahlVal>,
        payload_size: usize,
    ) -> Result<()> {
        let last_index = self.thread.stack.len() - payload_size;
    ...
    fn call_primitive_mut_func(
        &mut self,
        f: fn(&mut [StahlVal]) -> Result<StahlVal>,
        payload_size: usize,
    ) -> Result<()> {
        let last_index = self.thread.stack.len() - payload_size;
    ...
    fn call_primitive_func(
        ...
        // let result = self
        //     .thread
        //     .enter_safepoint(move |ctx: &StahlThread| f(&ctx.stack[last_index..]))
    ...
    fn call_future_func(
        &mut self,
        // f: Shared<Box<dyn Fn(&[StahlVal]) -> Result<FutureResult>>>,
        f: BoxedAsyncFunctionSignature,
        payload_size: usize,
    ) -> Result<()> {
        let last_index = self.thread.stack.len() - payload_size;

        let result = StahlVal::FutureV(Gc::new(f(&self.thread.stack[last_index..])?));
    ...
    // #[inline(always)]
    fn handle_function_call(&mut self, stack_func: StahlVal, payload_size: usize) -> Result<()> {
        use StahlVal::*;
...
pub fn current_function_span(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    if !args.is_empty() {
        builtin_stop!(ArityMismatch => format!("current-function-span requires no arguments, found {}", args.len()))
    }

    match ctx.enclosing_span() {
        Some(s) => Some(Span::into_stahlval(s)),
        None => Some(Ok(StahlVal::Void)),
    }
}

fn inspect_impl(ctx: &VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    let guard = ctx.thread.sources.sources.lock().unwrap();

    if let Some(StahlVal::Closure(c)) = args.get(0) {
...
#[stahl_derive::context(name = "inspect", arity = "Exact(1)")]
pub fn inspect(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
...
pub fn breakpoint(ctx: &mut VmCore, _args: &[StahlVal]) -> Option<Result<StahlVal>> {
    ...
    // having to bundle read line into stahl-core
...
#[stahl_derive::context(name = "call-with-exception-handler", arity = "Exact(2)")]
...
#[stahl_derive::context(name = "call/cc", arity = "Exact(1)")]
pub fn call_cc(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    /*
    ...
    */
    ...
    match function {
        ...
        StahlVal::ContinuationFunction(cc) => {
            Continuation::set_state_from_continuation(ctx, cc);

            ctx.ip += 1;
        }
        StahlVal::FuncV(f) => return Some(f(&[StahlVal::ContinuationFunction(continuation)])),
    ...
    Some(Ok(StahlVal::ContinuationFunction(continuation)))
}

fn eval_impl(ctx: &mut crate::stahl_vm::vm::VmCore, args: &[StahlVal]) -> Result<StahlVal> {
    let expr = crate::parser::ast::TryFromStahlValVisitorForExprKind::root(&args[0])?;
...
#[stahl_derive::function(name = "emit-expanded", arity = "Exact(1)")]
fn emit_expanded_file(path: String) {
    let mut engine = crate::stahl_vm::engine::Engine::new();
...
#[stahl_derive::function(name = "load-expanded", arity = "Exact(1)")]
fn load_expanded_file(path: String) {
    let mut engine = crate::stahl_vm::engine::Engine::new();
    engine.load_from_expanded_file(&path)
}

#[stahl_derive::context(name = "eval", arity = "Exact(1)")]
fn eval(ctx: &mut crate::stahl_vm::vm::VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
...
#[stahl_derive::context(name = "load", arity = "Exact(1)")]
fn eval_file(ctx: &mut crate::stahl_vm::vm::VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
...
#[stahl_derive::context(name = "eval-string", arity = "Exact(1)")]
fn eval_string(
...
#[stahl_derive::context(name = "#%expand", arity = "Exact(1)")]
...
fn expand_impl(ctx: &mut VmCore, args: &[StahlVal]) -> Result<StahlVal> {
...
fn eval_file_impl(ctx: &mut crate::stahl_vm::vm::VmCore, args: &[StahlVal]) -> Result<StahlVal> {
    let path = StahlString::from_stahlval(&args[0])?;
...
fn eval_string_impl(ctx: &mut crate::stahl_vm::vm::VmCore, args: &[StahlVal]) -> Result<StahlVal> {
    let string = StahlString::from_stahlval(&args[0])?;
...
pub(crate) fn get_test_mode(ctx: &mut VmCore, _args: &[StahlVal]) -> Option<Result<StahlVal>> {
    Some(Ok(ctx.thread.runtime_options.test.into()))
}

pub(crate) fn set_test_mode(ctx: &mut VmCore, _args: &[StahlVal]) -> Option<Result<StahlVal>> {
    ctx.thread.runtime_options.test = true;

    Some(Ok(ctx.thread.runtime_options.test.into()))
}

pub(crate) fn list_modules(ctx: &mut VmCore, _args: &[StahlVal]) -> Option<Result<StahlVal>> {
    use crate::rvals::AsRefStahlVal;
    ...
    Some(Ok(StahlVal::ListV(modules)))
}

pub(crate) fn environment_offset(ctx: &mut VmCore, _args: &[StahlVal]) -> Option<Result<StahlVal>> {
    Some(Ok(ctx.thread.global_env.len().into_stahlval().unwrap()))
}
...
pub(crate) fn expand_syntax_case_impl(_ctx: &mut VmCore, args: &[StahlVal]) -> Result<StahlVal> {
    if args.len() != 3 {
        stop!(ArityMismatch => format!("#%expand-template expected 3 arguments, found: {}", args.len()))
    }

    let mut bindings: fxhash::FxHashMap<_, _> = if let StahlVal::HashMapV(h) = &args[1] {
        h.iter()
            .map(|(k, v)| match (k, v) {
                (StahlVal::SymbolV(k), _e) => Ok((
                    InternedString::from_str(k.as_str()),
                    crate::parser::ast::TryFromStahlValVisitorForExprKind::root(v)?,
                )),
    ...
    let mut binding_kind: fxhash::FxHashMap<_, _> = if let StahlVal::HashMapV(h) = &args[2] {
        h.iter()
            .map(|(k, v)| match (k, v) {
                (StahlVal::SymbolV(k), e) => Ok((
                    InternedString::from_str(k.as_str()),
                    if usize::from_stahlval(e)? == 1 {
                        BindingKind::Many
                    } else {
                        BindingKind::Single
                    },
    ...
    let mut template = crate::parser::ast::TryFromStahlValVisitorForExprKind::root(&args[0])?;
...
#[stahl_derive::context(name = "#%expand-syntax-case", arity = "Exact(3)")]
pub(crate) fn expand_syntax_case(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    Some(expand_syntax_case_impl(ctx, args))
}

pub(crate) fn match_syntax_case_impl(ctx: &mut VmCore, args: &[StahlVal]) -> Result<StahlVal> {
    let macro_name: InternedString = String::from_stahlval(&args[0])?.into();
    ...
    let expr = crate::parser::ast::TryFromStahlValVisitorForExprKind::root(&args[1])?;
    ...
    let map = bindings
        ...
        .map(|(k, v)| {
            (
                StahlVal::SymbolV(k.resolve().trim_start_matches("##").into()),
    ...
    let kind = binding_kind
        ...
        .map(|(k, v)| {
            (
                StahlVal::SymbolV(k.resolve().trim_start_matches("##").into()),
                match v {
                    crate::parser::expander::BindingKind::Single => 0.into_stahlval().unwrap(),
                    crate::parser::expander::BindingKind::Many => 1.into_stahlval().unwrap(),
                },
            )
        })
        .collect::<crate::values::HashMap<_, _>>();
    ...
    Ok(crate::list![
        ...
        index.into_stahlval()?,
        StahlVal::HashMapV(Gc::new(map).into()),
        StahlVal::HashMapV(Gc::new(kind).into())
    ])
}

fn macro_case_bindings_impl(ctx: &mut VmCore, args: &[StahlVal]) -> Result<StahlVal> {
    let macro_name = String::from_stahlval(&args[0])?.into();
    let guard = ctx.thread.compiler.read();
        ...
        macro_object
            ...
            .map(|x| {
                StahlVal::ListV(
                    x.all_bindings()
                        .into_iter()
                        .map(|x| StahlVal::SymbolV(x.trim_start_matches("##").into()))
                        .collect(),
                )
            })
            .collect(),
...
#[stahl_derive::context(name = "#%macro-case-bindings", arity = "Exact(2)")]
pub(crate) fn macro_case_bindings(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    Some(macro_case_bindings_impl(ctx, args))
}

#[stahl_derive::context(name = "#%match-syntax-case", arity = "Exact(2)")]
pub(crate) fn match_syntax_case(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
...
///```
#[stahl_derive::context(name = "apply", arity = "Exact(2)")]
pub(crate) fn apply(ctx: &mut VmCore, args: &[StahlVal]) -> Option<Result<StahlVal>> {
    let current_instruction = ctx.instructions[ctx.ip - 1];
...
fn returns_value(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
    ...
}

if its a bin op, make it look like this:

fn bin_op(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
    ...
}
...
#[cfg(feature = "dynamic")]
fn num_equal_handler_no_stack(_ctx: &mut VmCore<'_>, l: StahlVal, r: StahlVal) -> Result<bool> {
    if let StahlVal::BoolV(b) = number_equality(&l, &r)? {
        Ok(b)
    } else {
        unreachable!()
    }
...
fn handle_load_int0(ctx: &mut VmCore<'_>) -> Result<()> {
    ctx.thread.stack.push(StahlVal::INT_ZERO);
    ctx.ip += 1;
    Ok(())
}
...
fn handle_load_int1(ctx: &mut VmCore<'_>) -> Result<()> {
    ctx.thread.stack.push(StahlVal::INT_ONE);
    ctx.ip += 1;
    Ok(())
}
...
fn handle_load_int2(ctx: &mut VmCore<'_>) -> Result<()> {
    ctx.thread.stack.push(StahlVal::INT_TWO);
    ctx.ip += 1;
    Ok(())
}
...
fn cons_handler(ctx: &mut VmCore<'_>) -> Result<()> {
    handler_inline_primitive_payload!(ctx, stahl_cons, 2);
    Ok(())
}
...
fn new_box_handler(ctx: &mut VmCore<'_>) -> Result<()> {
    ...
    let result = StahlVal::HeapAllocated(allocated_var);
...
fn unbox_handler(ctx: &mut VmCore<'_>) -> Result<()> {
    handler_inline_primitive_payload!(ctx, stahl_unbox_mutable, 1);
    Ok(())
}

fn setbox_handler(ctx: &mut VmCore<'_>) -> Result<()> {
    handler_inline_primitive_payload!(ctx, stahl_set_box_mutable, 2);
    Ok(())
}
...
fn not_handler(ctx: &mut VmCore<'_>) -> Result<()> {
    handler_inline_primitive_payload_1!(ctx, stahl_not);
    Ok(())
}
...
mod handlers {
    ...
    pub fn lte_binop(l: StahlVal, r: isize) -> bool {
        match l {
            StahlVal::IntV(l) => l <= r,
            _ => false,
        }
    ...
    pub fn sub_binop(l: StahlVal, r: isize) -> Result<StahlVal> {
        match l {
            StahlVal::IntV(l) => Ok(StahlVal::IntV(l - r)),
            StahlVal::NumV(l) => Ok(StahlVal::NumV(l - r as f64)),
    ...
    #[inline(always)]
    fn handle_push_no_stack(ctx: &mut VmCore<'_>, index: usize) -> Result<StahlVal> {
    ...
    #[inline(always)]
    fn handle_local_no_stack(ctx: &mut VmCore<'_>, index: usize) -> Result<StahlVal> {
        let offset = ctx.get_offset();
    ...
    #[inline(always)]
    fn handle_read_captures_no_stack(ctx: &mut VmCore<'_>, index: usize) -> Result<StahlVal> {
    ...
    #[inline(always)]
    fn handle_move_local_no_stack(ctx: &mut VmCore<'_>, index: usize) -> Result<StahlVal> {
        // let offset = self.stack_frames.last().map(|x| x.index).unwrap_or(0);
    ...
    #[inline(always)]
    fn handle_move_local_0_no_stack(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
        // let offset = self.stack_frames.last().map(|x| x.index).unwrap_or(0);
        // let offset = ctx.stack_frames.last().unwrap().index;
        let offset = ctx.get_offset();
    ...
    #[inline(always)]
    fn handle_move_local_1_no_stack(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
        // let offset = self.stack_frames.last().map(|x| x.index).unwrap_or(0);
        // let offset = ctx.stack_frames.last().unwrap().index;
        let offset = ctx.get_offset();
    ...
    #[inline(always)]
    pub fn handle_local_0_no_stack(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
        let offset = ctx.get_offset();
        let value = ctx.thread.stack[offset].clone();
        ctx.ip += 1;
        Ok(value)
    ...
    #[inline(always)]
    pub fn handle_local_1_no_stack(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
        let offset = ctx.get_offset();
    ...
    #[inline(always)]
    pub fn handle_local_2_no_stack(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
        let offset = ctx.get_offset();
    ...
    fn void_handler(ctx: &mut VmCore<'_>) -> Result<()> {
        ctx.thread.stack.push(StahlVal::Void);
        ctx.ip += 1;
        Ok(())
    }
    ...
    pub fn push_const_handler_no_stack(ctx: &mut VmCore<'_>) -> Result<StahlVal> {
        let payload_size = ctx.instructions[ctx.ip].payload_size;
        let val = ctx.constants.get_value(payload_size.to_usize());
        ctx.ip += 1;
        Ok(val)
    ...
    #[inline(always)]
    pub fn raw_if_handler(ctx: &mut VmCore<'_>, arg: StahlVal) {
        let payload_size = ctx.instructions[ctx.ip].payload_size;
        // change to truthy...
        if arg.is_truthy() {
            ctx.ip += 1;
    ...
    #[inline(always)]
    fn sub_handler_int_none(_: &mut VmCore<'_>, l: isize, r: StahlVal) -> Result<StahlVal> {
        match r {
            StahlVal::IntV(n) => Ok(StahlVal::IntV(l - n)),
            StahlVal::NumV(r) => Ok(StahlVal::NumV(l as f64 - r)),
            _ => stop!(TypeMismatch => "sub expected a number, found: {}", r),
        }
    }

    #[inline(always)]
    fn sub_handler_none_int(_: &mut VmCore<'_>, l: StahlVal, r: isize) -> Result<StahlVal> {
        match l {
            StahlVal::IntV(l) => Ok(StahlVal::IntV(l - r)),
            StahlVal::NumV(l) => Ok(StahlVal::NumV(l - r as f64)),
            _ => {
    ...
    #[inline(always)]
    fn multiply_handler_int_none(_: &mut VmCore<'_>, l: isize, r: StahlVal) -> Result<StahlVal> {
        match r {
            StahlVal::NumV(r) => Ok(StahlVal::NumV(l as f64 * r)),
            StahlVal::IntV(n) => {
                if let Some(res) = l.checked_mul(n) {
                    Ok(StahlVal::IntV(res))
                } else {
                    let mut res = num::BigInt::from(l);
                    res *= n;
                    Ok(StahlVal::BigNum(Gc::new(res)))
                }
            }
    ...
    #[inline(always)]
    fn lte_handler_none_int(_: &mut VmCore<'_>, l: StahlVal, r: isize) -> Result<bool> {
        match l {
            StahlVal::IntV(l) => Ok(l <= r),
            StahlVal::NumV(l) => Ok(l <= r as f64),
            _ => stop!(TypeMismatch => "lte expected an number, found: {}", r),
        }
    }

    #[inline(always)]
    pub(crate) fn add_handler_none_none(l: &StahlVal, r: &StahlVal) -> Result<StahlVal> {
        add_two(l, r)
    }
}
