...
fn derive_stahl_impl(input: DeriveInput, prefix: proc_macro2::TokenStream) -> TokenStream {
        ...
        Data::Struct(s) => {
                    ...
                    if should_impl_getters {
                        for field in s.fields.iter() {
                            ...
                            if let Some(field_name) = &field.ident {
                                ...
                                values.push(quote! {
                                    |value: &#name| {
                                        use #prefix::rvals::IntoStahlVal;
                                        &value.#field_name.clone().into_stahlval()
                                    }
                                });
                            }
                        }
                    ...
                    if should_impl_getters {
                        for (index, field) in s.fields.iter().enumerate() {
                            ...
                            values.push(quote! {
                                |value: &#name| {
                                    use #prefix::rvals::IntoStahlVal;
                                    &value.#index.clone().into_stahlval()
                                }
                            });
                        }
                    }
            ...
            let gen = quote! {
                ...
                impl #name {
                    ...
                    fn register_type(module: &mut #prefix::stahl_vm::builtin::BuiltInModule) ->
                        &mut #prefix::stahl_vm::builtin::BuiltInModule {
                        #(
        ...
        Data::Enum(e) => {
            ...
            'variant: for variant in &e.variants {
                ...
                match variant.fields {
                    syn::Fields::Named(_) => {
                        ...
                        values.push(quote! {
                        |value: #prefix::rvals::StahlVal| {
                            use #prefix::gc::ShareableMut;
                            if let #prefix::rvals::StahlVal::Custom(c) = value {
                                ...
                                if let Some(field_name) = &field.ident {
                                    ...
                                    values.push(quote! {
                                        |value: &#name| {
                                            use #prefix::rvals::IntoStahlVal;
                                            use #prefix::stop;
                                            use #prefix::rerrs::StahlErr;

                                            if let #name::#identifier { #field_name, .. } = &value {
                                                #field_name.clone().into_stahlval()
                                            } else {
            ...
            let gen = quote! {
                ...
                impl #name {
                    ...
                    fn register_enum_variants(module: &mut #prefix::stahl_vm::builtin::BuiltInModule) ->
                        &mut #prefix::stahl_vm::builtin::BuiltInModule {
                        #(
                            module.register_fn(#names, #values);
                        )*
...
#[proc_macro_derive(Stahl, attributes(stahl))]
pub fn derive_stahl(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let prefix = quote! { ::stahl };

    derive_stahl_impl(input, prefix)
}

#[proc_macro_derive(_Stahl, attributes(stahl))]
pub fn derive_stahl_internal(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    ...
    if let Some(doc_comments) = maybe_doc_comments {
        quote! {
            pub fn #function_name() -> BuiltInModule {
                ...
                module.register_doc(#value, crate::stahl_vm::builtin::MarkdownDoc(#doc_comments.into()));
...
pub fn native(
    ...
    let definition_struct = if let Some(doc) = maybe_doc_comments {
        quote! {
            pub const #doc_name: crate::stahl_vm::builtin::NativeFunctionDefinition =
            crate::stahl_vm::builtin::NativeFunctionDefinition {
                name: #value,
                aliases: &[],
                func: crate::stahl_vm::builtin::BuiltInFunctionType::Reference(#function_name),
                arity: crate::stahl_vm::builtin::Arity::#arity_number,
                doc: Some(crate::stahl_vm::builtin::MarkdownDoc::from_str(#doc)),
                is_const: #is_const,
                signature: None,
            };
        }
        ...
        quote! {
            pub const #doc_name: crate::stahl_vm::builtin::NativeFunctionDefinition = crate::stahl_vm::builtin::NativeFunctionDefinition {
                name: #value,
                aliases: &[],
                func: crate::stahl_vm::builtin::BuiltInFunctionType::Reference(#function_name),
                arity: crate::stahl_vm::builtin::Arity::#arity_number,
    ...
    let definition_struct = if let Some(doc) = maybe_doc_comments {
        quote! {
            pub const #doc_name: crate::stahl_vm::builtin::NativeFunctionDefinition = crate::stahl_vm::builtin::NativeFunctionDefinition {
                name: #value,
                aliases: &[],
                func: crate::stahl_vm::builtin::BuiltInFunctionType::Context(#function_name),
                arity: crate::stahl_vm::builtin::Arity::#arity_number,
                doc: Some(crate::stahl_vm::builtin::MarkdownDoc::from_str(#doc)),
                is_const: #is_const,
                signature: None,
    ...
    let definition_struct = if let Some(doc) = maybe_doc_comments {
        quote! {
            pub const #doc_name: crate::stahl_vm::builtin::NativeFunctionDefinition = crate::stahl_vm::builtin::NativeFunctionDefinition {
                name: #value,
                aliases: &[],
                func: crate::stahl_vm::builtin::BuiltInFunctionType::Mutable(#function_name),
                arity: crate::stahl_vm::builtin::Arity::#arity_number,
                doc: Some(crate::stahl_vm::builtin::MarkdownDoc::from_str(#doc)),
                is_const: #is_const,
                signature: None,
...
pub fn function(
            ...
            if let Type::Path(val) = *r {
                ...
                if let Some(last) = last {
                    match last.ident.into_token_stream().to_string().as_str() {
                        "Result" => quote! { res },
                        _ => quote! {
                            res.into_stahlval().map_err(err_thunk)
                        },
                    }
                } else {
                    quote! {
    ...
    for (i, arg) in sign.inputs.iter().enumerate() {
        if let FnArg::Typed(pat_ty) = arg.clone() {
            ...
            match pat_ty.ty.clone().into_token_stream().to_string().as_str() {
                ...
                "StahlString" => argument_signatures.push("string"),
                "&StahlString" => argument_signatures.push("string"),
    ...
    let copied_function_name = Ident::new(
        &("stahl_".to_string() + &function_name.to_string()),
        sign.ident.span(),
    );
    ...
    let function_type = if promote_to_mutable {
        quote! {
            crate::stahl_vm::builtin::BuiltInFunctionType::Mutable(#copied_function_name)
        }
    } else {
        quote! {
            crate::stahl_vm::builtin::BuiltInFunctionType::Reference(#copied_function_name)
        }
    };
    ...
    let definition_struct = if let Some(doc) = maybe_doc_comments {
        quote! {
            pub const #doc_name: crate::stahl_vm::builtin::NativeFunctionDefinition = crate::stahl_vm::builtin::NativeFunctionDefinition {
                ...
                aliases: #aliases,
                func: #function_type,
                arity: crate::stahl_vm::builtin::Arity::#arity_exactness(#arity_number),
                doc: Some(crate::stahl_vm::builtin::MarkdownDoc::from_str(#doc)),
                is_const: #is_const,
                signature: None,
            };
    ...
    if rest_arg_generic_inner_type {
        ...
        let output = quote! {
            ...
            pub fn #copied_function_name(args: &[StahlVal]) -> std::result::Result<StahlVal, crate::rerrs::StahlErr> {

                use crate::rvals::{IntoStahlVal, FromStahlVal, PrimitiveAsRef};

                if args.len() < #arity_number {
                    crate::stop!(ArityMismatch => format!("{} expected {} arguments, got {}", #value, #arity_number.to_string(), args.len()))
                }

                fn err_thunk(mut err: crate::rerrs::StahlErr) -> crate::rerrs::StahlErr {
    ...
    if promote_to_mutable {
        ...
        let output = quote! {
                ...
                pub fn #copied_function_name(args: &mut [StahlVal]) -> std::result::Result<StahlVal, crate::rerrs::StahlErr> {

                    use crate::rvals::{IntoStahlVal, FromStahlVal, PrimitiveAsRef, PrimitiveAsRefMut};
        ...
        // }
                    ...
                    //         // TODO: Distinguish reference types here if possible - make a special implementation
                    //         // for builtin pointer types here to distinguish them
                    //         <#arg_type>::#conversion_functions(&mut std::mem::replace(&mut args[#arg_index], StahlVal::Void))
                    //             .map_err(err_thunk)?,
                    //     )*
                    // );
