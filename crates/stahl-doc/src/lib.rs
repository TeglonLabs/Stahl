...
use stahl::compiler::modules::MANGLER_PREFIX;
...
fn walk_for_defines<W: Write>(
    writer: &mut W,
    ast: &[stahl::parser::ast::ExprKind],
) -> Result<(), Box<dyn Error>> {
    let mut nodes = ast.iter();

    while let Some(node) = nodes.next() {
        match &node {
            stahl::parser::ast::ExprKind::Define(d) => {
                ...
                if !name.starts_with(MANGLER_PREFIX) && name.ends_with("__doc__") {
                    ...
                    if let stahl::parser::ast::ExprKind::Define(def) = &ast_node {
                        if let stahl::parser::ast::ExprKind::Quote(q) = &def.body {
                            if let stahl::parser::ast::ExprKind::Define(d) = &q.expr {
                                if let stahl::parser::ast::ExprKind::LambdaFunction(l) = &d.body {
            ...
            stahl::parser::ast::ExprKind::Begin(b) => {
...
pub fn parse_cog_file(path: PathBuf) -> stahl::rvals::Result<String> {
    let contents = std::fs::read_to_string(path)?;
    let exprs = stahl::parser::parser::Parser::parse(&contents)?;
    todo!()
}
