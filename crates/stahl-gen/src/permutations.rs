...
impl ValueKind {
    ...
    fn as_from_trait_name(&self) -> &'static str {
        match self {
            Self::Value => "FromStahlVal",
            Self::Reference => "AsRefStahlVal",
            Self::MutReference => "AsRefMutStahlVal",
        }
    }
}
...
fn code_gen_permutation(values: &[ValueKind], id: usize, row: usize) -> String {
    ...
    impl_trait.bound(RET, "IntoStahlVal"); // TODO: Figure out if these bounds are respected or not
    for (i, value) in values.iter().enumerate() {
    ...
    function_def.line("let f = move |args: &[StahlVal]| -> Result<StahlVal> {");
    ...
    for (i, value) in values.iter().enumerate() {
        match value {
            ValueKind::Value => {
                function_def.line(format!(
                    "let mut v{i} = <T{i}>::from_stahlval(&args[{i}])?;"
                ));
            }
            ValueKind::Reference => {
                function_def.line(format!(
                    "let mut nursery{i} = <T{i} as AsRefStahlVal>::Nursery::default();"
                ));
                function_def.line(format!(
...

    {
        ...
        function_def.line("res.into_stahlval()");
    }
