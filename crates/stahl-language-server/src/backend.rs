...
use stahl::{
    compiler::{
        modules::{stahl_home, MANGLER_PREFIX, MODULE_PREFIX},
        passes::analysis::{
    ...
    parser::{
        ast::ExprKind, expander::StahlMacro, interner::InternedString, parser::SourceId,
        span::Span, tryfrom_visitor::SyntaxObjectFromExprKindRef,
    },
    rvals::{FromStahlVal, StahlString},
    stahl_vm::{builtin::BuiltInModule, engine::Engine, register_fn::RegisterFn},
};
...
pub fn lsp_home() -> PathBuf {
    if let Ok(home) = std::env::var("STAHL_LSP_HOME") {
        return PathBuf::from(home);
    }

    let mut home_directory =
        PathBuf::from(stahl_home().expect("Unable to find stahl home location"));
...
pub struct Backend {
    ...
    pub _macro_map: DashMap<String, HashMap<InternedString, StahlMacro>>,
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let completions = || -> Option<Vec<CompletionItem>> {
            ...
            for context in contexts {
                match context {
                    stahl::compiler::passes::analysis::SemanticInformationType::Function(info) => {
                        completions.extend(
                            info.arguments()
                                .iter()
                                .map(|x| &x.0)
                    ...
                    stahl::compiler::passes::analysis::SemanticInformationType::Let(info) => {
    ...
    async fn on_change(&self, params: TextDocumentItem) {
        ...
        let diagnostics = {
            let program = {
                ...
                let mut introduced_macros: HashMap<InternedString, StahlMacro> = HashMap::new();
...
fn uri_to_source_id(uri: &Url) -> Option<stahl::parser::parser::SourceId> {
    let id = ENGINE
        .read()
        .ok()?
    ...
    pub fn new(
        ...
        for file in std::fs::read_dir(&directory)? {
            ...
            if file.path().is_file() {
                ...
                for value in result {
                    if let Ok(module) = BuiltInModule::from_stahlval(&value) {
...
impl stahl::stahl_vm::engine::ModuleResolver for ExternalModuleResolver {
...
pub static DIAGNOSTICS: Lazy<RwLock<Vec<StahlDiagnostic>>> = Lazy::new(|| RwLock::new(Vec::new()));
...
fn configure_lints() -> std::result::Result<UserDefinedLintEngine, Box<dyn Error>> {
    ...
    diagnostics.register_fn("suggest", move |span: Span, message: StahlString| {
        // log::debug!("Adding suggestion at: {:?} - {:?}", span, message);
        DIAGNOSTICS
            .write()
            .unwrap()
            .push(StahlDiagnostic { span, message });
    });
    ...
    if let Ok(directory) = std::fs::read_dir(directory) {
        ...
        log::info!("Missing lints directory in $STAHL_LSP_HOME");
    }
