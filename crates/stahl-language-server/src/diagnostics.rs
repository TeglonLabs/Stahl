...
use stahl::{
    ...
    stahl_vm::{builtin::Arity, engine::Engine},
    stop, StahlVal,
};
...
impl<'a> VisitorMutUnitRef<'a> for StaticArityChecking<'a> {
    fn visit_begin(&mut self, begin: &'a stahl::parser::ast::Begin) {
...
impl<'a, 'b> VisitorMutUnitRef<'a> for StaticCallSiteArityChecker<'a, 'b> {
    fn visit_list(&mut self, l: &'a stahl::parser::ast::List) {
                    ...
                    if info.builtin {
                        let found_arity = self
                            ...
                            .or_else(|| {
                                ...
                                if let StahlVal::BoxedFunction(b) = self
                                    .context
                                    .engine
...
define_symbols! {
    BIND_C => "bind/c",
    PRIM_BIND_C => "mangler#%private/stahl/contract__%#__bind/c",
    MAKE_FUNCTION_C => "make-function/c",
    PRIM_MAKE_FUNCTION_C => "mangler#%private/stahl/contract__%#__make-function/c",
    MAKE_C => "make/c",
    PRIM_MAKE_C => "mangler#%private/stahl/contract__%#__make/c",
    LIST_OF => "listof",
    PRIM_LIST_OF => "mangler#%private/stahl/contract__%#__listof",
}
...
fn function_contract(expr: &ExprKind) -> Option<stahl::rvals::Result<StaticContract>> {
    let body = expr.list()?;
...
impl StaticContract {
    ...
    fn from_exprkind(expr: &ExprKind) -> stahl::rvals::Result<StaticContract> {
        match expr {
            ExprKind::Atom(a) => {
            ...
            ExprKind::List(l) => match l.first_ident() {
                ...
                Some(x) if is_make_function_c(x) => {
                    ...
                    let pre_condition_contracts = pre_conditions
                        ...
                        .collect::<stahl::rvals::Result<Vec<_>>>()?;
...
impl<'a> VisitorMutUnitRef<'a> for GlobalContractCollector {
    ...
    fn visit_set(&mut self, s: &'a stahl::parser::ast::Set) {
        if let Some(contract) = function_contract(&s.expr) {
