...
use stahl::{
    rvals::{Custom, SerializableStahlVal},
    stahl_vm::ffi::{
...
impl Custom for CommandMessenger {
    fn into_serializable_stahlval(&mut self) -> Option<stahl::rvals::SerializableStahlVal> {
        Some(stahl::rvals::SerializableStahlVal::Custom(Box::new(
            self.clone(),
        )))
    }
}
...
pub fn build_module() -> FFIModule {
    let mut module = FFIModule::new("dylib/stahl/webserver");
