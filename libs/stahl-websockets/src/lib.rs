...
use stahl::stahl_vm::ffi::{as_underlying_ffi_type, CustomRef, FFIArg, FFIValue};
use tungstenite::{
    connect, handshake::client::Response, stream::MaybeTlsStream, Message, WebSocket,
};

use stahl::{rvals::Custom, stahl_vm::ffi::IntoFFIVal};
...
struct StahlWebSocket(WebSocket<MaybeTlsStream<TcpStream>>);
impl Custom for StahlWebSocket {}
...
/// In lieu of a direct bytes type in Stahl, since we need to typically respond to a ping with a pong
/// containing the matching payload, we can skip the deserializing into the Stahl type and directly
...
fn _make_non_blocking(socket: &mut StahlWebSocket) -> std::result::Result<(), std::io::Error> {
...
fn create_module() -> FFIModule {
    let mut module = FFIModule::new("stahl/web/ws");
    ...
    module
        ...
        .register_fn("ws/connect", |url: String| {
            connect(url)
                .map(|(socket, resp)| {
                    vec![
                        StahlWebSocket(socket).into_ffi_val().unwrap(),
                        WebSocketResponse(resp).into_ffi_val().unwrap(),
                    ]
                })
                .map_err(WebSocketError)
        })
        .register_fn("ws/read-message!", StahlWebSocket::read_message)
        .register_fn("ws/write-message!", StahlWebSocket::write_message);
