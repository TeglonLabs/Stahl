...
(define (scheme-add-macro name proc)
  (set! scheme-global-environment
        (cons (car scheme-global-environment)
...
(define (comp-quasiquotation form level env)
  (cond
    ...
    [(pair? form)
     (cond
       [(eq? (car form) 'quasiquote) (comp-quasiquotation-list form (+ level 1) env)]
       [(eq? (car form) 'unquote)
        (if (= level 1)
    ...
    [(vector? form) (gen-vector-form (comp-quasiquotation-list (vector->lst form) level env))]
    [else (gen-cst form)]))
...
(define (comp-body body env)
  ...
  (define (letrec-defines vars vals body env)
    (if (pair? body)
        ...
        (let ([expr (car body)])
          (cond
            [(not (pair? expr)) (letrec-defines* vars vals body env)]
            [(macro? (car expr) env)
             (letrec-defines vars vals (cons (macro-expand expr env) (cdr body)) env)]
            [else
             (cond
               [(eq? (car expr) 'begin) (letrec-defines vars vals (append (cdr expr) (cdr body)) env)]
               [(eq? (car expr) 'define)
                (let ([x (definition-name expr)])
                  (variable x)
                  (letrec-defines (cons x vars) (cons (definition-value expr) vals) (cdr body) env))]
               [(eq? (car expr) 'define-macro)
                (let ([x (definition-name expr)])
                  (letrec-defines vars
                                  vals
                                  (cdr body)
                                  (push-macro x (scheme-eval (definition-value expr)) env)))]
  ...
  (if (pair? clauses)
      (let ([clause (car clauses)])
        ...
        (cond
          ...
          [(not (pair? (cdr clause)))
           (gen-or (scheme-comp (car clause) env) (comp-cond-aux (cdr clauses) env))]
          [(eq? (cadr clause) '=>)
           (shape clause 3)
...
(define (comp-case expr env)
  (shape expr 3)
  (gen-case (scheme-comp (cadr expr) env) (comp-case-aux (cddr expr) env)))
      ...
      [(symbol? x)
       ...
       (let ([y (caddr expr)])
         (let ([proc (cons 'lambda (cons (bindings->vars y) (cdddr expr)))])
           (scheme-comp (cons (list 'letrec (list (list x proc)) x) (bindings->vals y)) env)))]
      [(pair? x)
       (scheme-comp (cons (cons 'lambda (cons (bindings->vars x) (cddr expr))) (bindings->vals x))
                    env)]
...
(define (bindings->vals bindings)
  (if (pair? bindings)
      (let ([binding (car bindings)]) (cons (cadr binding) (bindings->vals (cdr bindings))))
      '()))
...
(define (comp-let* expr env)
  ...
  (let ([bindings (cadr expr)])
    (if (pair? bindings)
        (scheme-comp (list 'let (list (car bindings)) (cons 'let* (cons (cdr bindings) (cddr expr))))
                     env)
...
(define (comp-letrec expr env)
  (shape expr 3)
  (let ([bindings (cadr expr)])
    (comp-letrec-aux (bindings->vars bindings) (bindings->vals bindings) (cddr expr) env)))

(define (comp-letrec-aux vars vals body env)
  (if (pair? vars)
      (let ([new-env (push-frame vars env)])
        (gen-letrec (comp-vals vals new-env) (comp-body body new-env)))
      (comp-body body env)))
...
(define (comp-do expr env)
  ...
  (let ([bindings (cadr expr)] [exit (caddr expr)])
    ...
    (let* ([vars (bindings->vars bindings)]
      ...
      (gen-letrec
       (list (gen-lambda (length vars)
                         (gen-if (scheme-comp (car exit) new-env2)
                                 (comp-sequence (cdr exit) new-env2)
                                 (gen-sequence (comp-sequence (cdddr expr) new-env2)
                                               (gen-combination (gen-var-ref '(1 . 1))
                                                                (comp-vals (bindings->steps bindings)
                                                                           new-env2))))))
       (gen-combination (gen-var-ref '(0 . 1)) (comp-vals (bindings->vals bindings) new-env1))))))
...
(define (comp-define-macro expr env)
  (let ([x (definition-name expr)]) (gen-macro x (scheme-eval (definition-value expr)))))
...
(define (scheme-comp expr env)
  (cond
    [(symbol? expr) (comp-var expr env)]
    [(not (pair? expr)) (comp-self-eval expr env)]
    [(macro? (car expr) env) (scheme-comp (macro-expand expr env) env)]
    [else
     (cond
...
;; Compatibility -- In order to use mutable pairs, this will
;; need to be adjusted slightly.

(define scheme-global-variables '())

(struct MutablePair (first second) #:mutable)
...
(define (scheme-global-var-ref i)
  (MutablePair-second i))

(define (scheme-global-var-set! i val)
  (set-MutablePair-second! i val)
  '())
  ...
  '(let ()
     ...
     (define (sort-list obj pred)
       ...
       (define (merge one two)
         (cond
           [(null? one) two]
           [(pred (car two) (car one)) (cons (car two) (merge (cdr two) one))]
